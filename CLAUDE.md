...
| `./tools/dev.ps1 test` | **既定**。tools の速いテスト + `verify-generated` + L1 + L3 | **約 2 分 13 秒**（増分、成果物が最新。2026-09-07 実測、exit 0。同じ条件で 2 回測って同じ値）。**M3.5 以前の「約 21 秒」→ 2026-09-01 の「約 65 秒」→ 2026-09-06 の「約 2 分 24 秒」から縮んだが、65 秒には戻っていない。** 2026-09-06 の回帰の主因だった `dotnet build`（`BindingGenerator.Tests.ps1` が**合成した非既定 profile の生成物を `dotnet build` に掛ける**検査、単体 36 秒）を `tools/tests/NonStandardProfileCompile.Tests.ps1` へ切り出し、`$ToolsTestScriptsSlow`（CI 専用）へ移した（2026-09-07、M7b）——検査は削っておらず、負の対照（`LibraryName` 宣言を欠かすと CS0103 で落ちること）も新居で実測済み。**それでも 65 秒へは戻らない**——切り出した後の `BindingGenerator.Tests.ps1` は単体でも 30 秒かかる（`native/include/ocvu/*.h` を書き換えて戻す負の対照が複数箇所あるため）。**しかもこの書き換えは native の再ビルドに伝播する**——ヘッダの mtime が変わるので、直後に `dev.ps1 test-native` を走らせると全 .cpp（20 本超）が再コンパイルされる（実測: `test-tools` の直後の `test-native` 単体は 9 秒 → 56 秒に伸びた）。この伝播は M7b 以前から在る構造で、今回の修正のスコープ外として残してある |
| `./tools/dev.ps1 test-tools` | `tools/tests/` の速いレーン（OpenCV 構成・ハッシュ無効化・**生成物と spec の一致**・**Unity と Emscripten の対応表**・**tarball の大きさ**・**binary の公開面が spec と一致すること（M7b）**・**benchmark の結果が publish できる形かどうか**）。**本数を写さない。正本は `tools/dev.ps1` の `$ToolsTestScriptsFast` である**（M7 で 2 本増えた。この列挙もそれに合わせて足した——列挙自体を数の代わりにしない） | 約 18 秒（**2 本だった頃の値**）。**2026-09-07 実測では約 69 秒**——`BindingGenerator.Tests.ps1` から `dotnet build` を伴う検査を抜いた後の値（下の `test` 行を見よ） |
...
「ローカルループは秒単位を死守する」という不変条件（本ファイル下部）と、`test` の実測（2026-09-07 で約 2 分 13 秒）はすでに緊張関係にある。M1 ではこれを**受け入れて記録するに留めており、解消していない**。**M6 でさらに 4 本目の `pwsh -NoProfile` 起動が加わったので、65 秒はもう上限ではない**（M3.5 のときと同じく、**前提が変わったことは記録し、再計測はしていない**）。**M5 でこの緊張は 1 段強まった** —— M1 の 65 秒は**ソースを変えた**ときの値だったが、2026-09-01 の 65 秒は**何も変えていない**ときの値である（tools のテストが 3 本になり、`verify-generated` と L3 の 2 つ目の assembly で `dotnet` の起動が 2 回増えた）。**M7b でさらに 1 段強まり、実測は 2 分 24 秒になった**（tools の速いテストが 7 本になり、そのうち 1 本は `dotnet build` を回す）。**その `dotnet build` を CI 専用の slow lane へ切り出す修正で 2 分 13 秒まで縮めたが、これで解消したわけではない**——`test-tools` 単体はまだ約 69 秒あり、しかもその一部（生成物ヘッダの書き換え・復元）は native の全 .cpp 再ビルドを誘発する副作用を持つ（上の `test` 行の実測を見よ）。着手するなら configure の結果を跨いで再利用するか、OpenCV に依存しないレーンを分けるか、負の対照がヘッダの mtime を動かさない形（内容を変えずに検査する、または元の mtime を明示的に復元する）に直すことになる。
