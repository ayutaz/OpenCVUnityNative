...
| `./tools/dev.ps1 test-tools` | `tools/tests/` の速いレーン（OpenCV 構成・ハッシュ無効化・**生成物と spec の一致**・**Unity と Emscripten の対応表**・**tarball の大きさ**・**配布 binary の公開面が spec と一致すること（M7b）**）。**本数を写さない。正本は `tools/dev.ps1` の `$ToolsTestScriptsFast` である**（M7 で 2 本増えた。この列挙もそれに合わせて足した——列挙自体を数の代わりにしない） | 約 18 秒（**2 本だった頃の値**） |
...
| `./tools/dev.ps1 test-unity-editmode` | L4（Unity EditMode、Mono、**57 件**。`-testCategory '!Graphics'` で `test-unity-graphics` 側のテストを除外する） | 約 27 秒（増分。2026-08-28 実測。**Unity 6000.0.82f1・EditMode 10 件のときの値。** その後 M3.5 で 16 件、M4 で 33 件、M5 で 34 件、2026-09 の API 拡張で 48 件、M7a で 53 件、M7b で `ProfileGatingTests`（4 件）が加わり 57 件になったが所要時間は取り直していない） |
...
| `Packages/com.ayutaz.opencv-unity-native/Runtime/Interop.Dnn/` | **非既定 profile（`dnn`）の宣言先（M7b）。** spec の `profile` が `dnn` の module があれば、生成物 `NativeMethods.<module>.g.cs` はここへ、クラス名は `NativeMethodsDnn`、assembly は `CvUnity.Interop.Dnn` になる（規約は `docs/abi-ownership-and-versioning.md` §4）。asmdef は `defineConstraints: ["OCVU_PROFILE_DNN"]` を持ち、既定ビルドには含まれない。**いまは中身が空である** —— `bindings/spec/dnn.json` がまだ無く、`profile: "dnn"` の module も無い。define を立てて Unity 自身に「実際にコンパイルされ、外すと消えるか」を問う検査は `tests/UnityProject/Assets/Tests/EditMode/ProfileGatingTests.cs` にある |
...
- **`RenderTexture` を使う公開 API（M7a）。** `RenderTextureConverter.ToMat` /
  `RequestMat` は **CI で 1 度も実行されない** —— 検証は `test-unity-graphics` が
  持つが、これはローカル専用で `ci-unity.yml` からは呼ばれず、
  `test-unity-player` / `ci-unity` の 2 レーンは `-nographics` で走るので
  この経路を通れない。**前 2 者（tarball / web）は「別の経路で同じものを
  見ている」レーンだが、こちらは配る package に入る公開 API がどの経路
  からも見られていない**という点で 1 段重い。`[Category("Graphics")]` を
  付けたテストの一覧は
