...
| `./tools/dev.ps1 generate` | **M5。** `bindings/spec/*.json` から C ヘッダ・C# の P/Invoke・到達性テスト・API 対応表を書き出す（**20 ファイル**）。**境界の宣言を手で書く経路はもう無い** | — |
...
| `./tools/dev.ps1 test-native` | L1 のみ（GoogleTest **215 件** + CTest **4 件**） | 約 10 秒（**`test-tools` の直後でも同じ**。2026-09-07 実測、9.0 秒 / 8.9 秒 / 8.8 秒。M7b で mtime 復元を直す前は、直前に `test-tools` を走らせると 56〜57 秒に伸びていた——ヘッダの mtime が新しいままだったため） |
| `./tools/dev.ps1 test-managed` | L3 のみ。**solution の全テストプロジェクトを回す** —— `CvUnity.Tests.Managed` が P/Invoke 越しに実物の DLL を叩く **185 件**（M7a で `AllocationTests` が加わり 181 → 185）、`Ocvu.Generator.Tests` が spec と生成器を見る **116 件**（M5 で新設、106 件。M7b で profile 分岐のテストが加わり 113 件、最終レビューの C-1 で `LibraryName` の写しを見る 3 件が加わり 116 件になった）。**件数を書いてあるのはこの行だけである** —— 他所に写すと、テストを 1 件足した瞬間にそちらだけが嘘になる（M5 で実際に 4 箇所が同時に古くなった） | 約 11 秒（**44 件だけだった頃の値**） |
...
| `docs/api-reference.md` | C ABI と C# 公開 API（`CvMat`/`CvOps`/`CvCodecs`/`CvQrCode`/`CvFeatures`/`CvGeometry`/`CvCalibration`/**`CvAruco`**/**`CvCoreOps`**/**`CvStereo`**/`CvNative`/`TextureConverter`/`WebCamTextureConverter`/**`RenderTextureConverter`**/`NativeArrayExtensions`）のリファレンス（M3 Task 7。M3.5 で `ocvu_imencode` / `ocvu_imdecode` と `CvCodecs` が加わった。M5 で `CvCalibration` が加わった。M7a で `RenderTextureConverter` が加わった） |
