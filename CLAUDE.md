...
| `./tools/dev.ps1 test-unity-player` | L5（Unity IL2CPP Player のビルドと実行、**35 件**。`-nographics` で走るので GPU に依る経路は含まない） | 約 54 秒（増分・キャッシュ温状態。2026-08-28 実測。cold 実測はまだ無く、roadmap の想定は 5〜20 分。**Unity 6000.0.82f1・10 件での値**） |
| `./tools/dev.ps1 test-unity-graphics` | **M7a。ローカル専用**。`RenderTexture` を使う GPU 依存の経路（`GraphicsTests` / `GraphicsBenchmarkRunner`）を EditMode で検証する。`-nographics` を**付けない**（`-nographics` の下では `graphicsDeviceType` が `Null` になり `RenderTexture.Create()` が true を返しつつ画素が読めなくなるため、実測）。**`ci-unity.yml` からは呼ばれない** —— このレーンが赤くても merge は止まらない。CI から見えないテストの一覧は `tests/UnityProject/Assets/Tests/EditMode/CiVisibilityTests.cs` が名指しで固定している（**7 件**） | 未計測 |
...
| `./tools/dev.ps1 benchmark` | **M7a。** `test-unity-player` と `test-unity-graphics` を順に走らせ、両方の結果 XML から `OCVU_BENCH:` 行（`TestContext.WriteLine` が結果 XML の `<output>` に入れたもの）を集めて `artifacts/benchmarks/latest.json` へ書く。**時間を判定しない** —— 落ちるのは「測れなかったとき」（0 マイクロ秒）だけで、遅い・速いでは落ちない（設計 D1）。数字の読み方は [性能](docs/performance.md) にまとめてある | 未計測（内部で呼ぶ 2 レーンの合計） |
...
| `tools/run-benchmarks.ps1` | Unity の結果 XML から `OCVU_BENCH:` 行を集めて `artifacts/benchmarks/latest.json` へ書く（M7a）。**判定しない** —— 時間の閾値は共有ランナーの上でフレークになるので、「測れたか」（0 マイクロ秒でないか）だけを見る（設計 D1）。**ログではなく結果 XML の `//output` を読む** —— このリポジトリの確立した型は `TestContext.WriteLine` で、NUnit3 がそれを結果 XML の `<output>` ノードへ入れる |
...
| `docs/api-reference.md` | C ABI と C# 公開 API（`CvMat`/`CvOps`/`CvCodecs`/`CvQrCode`/`CvFeatures`/`CvGeometry`/`CvCalibration`/**`CvAruco`**/**`CvCoreOps`**/**`CvStereo`**/`CvNative`/`TextureConverter`/`WebCamTextureConverter`/`NativeArrayExtensions`）のリファレンス（M3 Task 7。M3.5 で `ocvu_imencode` / `ocvu_imdecode` と `CvCodecs` が加わった。M5 で `CvCalibration` が加わった） |
