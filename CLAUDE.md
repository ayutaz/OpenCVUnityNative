...
**配った実績は 5 つある: v0.1.0（2026-08-28）、v0.1.1（2026-08-29）、v0.2.0（2026-08-30）、v0.3.0（2026-09-04）、v0.4.0（2026-09-12、最新の公開版）。** いずれも tag から `release.yml` が**その時点の全 platform 分**を作り、`--draft` で下書きにしてから人が公開している。v0.1.1 は上に書いた Linux の欠陥を直した版で、**中身を差し替えず新しい版として出した** — 一度配ったものを黙って差し替えると、同じ版名で違う物が世の中に 2 つ存在することになる。asset は 16 件（3 platform × 5 + `SHA256SUMS.txt`）。**これは v0.1.1 までの形である。その後 M3.5 / M4 / M6 で増えた**（現在の数は下の `release.yml` の行が持つ）。**公開された物を落として実測した（2026-08-29、このマシン）**: Linux の tarball は `SHA256SUMS.txt` と一致し、中の `libopencv_unity_native.so` は `GLIBC<=2.34, GLIBCXX<=3.4.29`（上限 2.35 / 3.4.30）だった。
...
3. **Release asset 名の衝突。** 3 platform が同じ `checksums.txt` 等を出すので、そのまま渡すと上書きされる。platform 名を頭に付け、staging した数を数えて確かめる（当時 15 件 = 3 × 5。**現在の数は下の `release.yml` の行が持つ** —— ここに写すと platform が増えた日にこの行だけが嘘になる）。
...
**M4 は完了していない。完了条件 9 件のうち 6 件を満たし、3 件は閉じていない**（判定表は `docs/roadmap.md` の M4 節が正本）。**2026-09-01 に、残る条件をすべてスキップすると決めた** ——iOS / Android の実機、lifecycle と memory pressure、macOS 上での Unity 起動、Windows IL2CPP の結論。**実機が要る 2 件は CI では原理的に閉じない**ので、人が実行する手順書（`docs/m4-device-verification.md`）に落としてある。**この 4 件はどれも自然には閉じない** —— **「まだ調べていない」ではなく「やらないと決めた」である**（判断と帰結は `docs/roadmap.md` の「配布 その 4」に 1 箇所だけ書いてある）。帰結: **Android / iOS は「CI がビルドするが誰も動かしたことがない」まま残っている。**（**利用者に届く最新版は v0.4.0（6 platform、2026-09-12 公開）である** —— 2026-09-01 の時点では v0.2.0 のままだったが、その後 v0.3.0 と v0.4.0 を配った。**実機で動かしていないことは、どちらの版でも変わっていない。**）判定表は `docs/roadmap.md` の M4 節。
...
**v0.3.0 は 2026-09-04 に公開した**（asset 33 件）。**M4 / M5 / M6 の成果が、これで初めて利用者に届いた** —— v0.2.0 以来である。**実機で一度も動かしていないことは変わらない**ので、その穴はリリースノートに明記してある（経緯は `docs/roadmap.md` の「配布 その 5」）。

**v0.4.0 は 2026-09-12 に公開した。** **M7（`dnn` の opt-in profile / `RenderTexture` の低コピー経路 / native と C# の module 分離）と 2026-09 の API 拡張の成果が、これで初めて利用者に届いた** —— v0.3.0 以来である。**退行を 1 つ含む**: PNG の encode / decode が Android / iOS / macOS arm64 で ARM 加速を失った（上流 OpenCV 5.0.0 の vendoring の欠陥を避けるための取引で、JPEG と `core` / `imgproc` の SIMD には触れていない）。**実機で一度も動かしていないことは、この版でも変わらない。** 経緯は `docs/roadmap.md` の「配布 その 6」。

**M5（binding specification と generator）は実装が済み、CI も green になった**（PR #55、2026-09-01。必須 21 本すべて）**。境界の宣言はもう手で書かない。** `bindings/spec/*.json` が正本で（**module 名を写さない**）、そこに並ぶ entry から **C ABI 宣言（`native/include/ocvu/*.h`）／C# の P/Invoke（`Runtime/Interop/NativeMethods.<module>.g.cs`）／全 entry point を 1 回ずつ呼ぶ到達性テスト（`tests/UnityProject/Assets/Tests/Shared/AbiReachabilityChecks.g.cs`）／API 対応表（`docs/api-map.md`）**が同時に出る（`./tools/dev.ps1 generate`）。**本数を数えるのは `docs/api-map.md` の冒頭だけである** —— ここに書くと、ABI が 1 本増えたときにこの行だけが静かに嘘になる。**手書きの `[DllImport]` は 0 個である**（全部が `.g.cs` の中にある）。一致は `./tools/dev.ps1 verify-generated` が見て、これは `dev.ps1 test` に入っているので **3 platform の `ci-native` が走らせる**（**PR #55 で 3 platform とも実際に通った**）。**検査は両向きに効く** —— 生成物を手で触ると落ち（実測）、`native/src/**/*.cpp` に `extern "C"` で `ocvu_` を実装して spec に書かなければ落ちる（後者は `tools/tests/BindingGenerator.Tests.ps1`。**見ているのは `native/src` の `.cpp` だけ**なので、他所に定義を置くと網に入らない）。**ABI は 1 本も増減していない** —— `OCVU_ABI_VERSION` は 1 のままで、M5 は移設であって追加ではない。**完了条件 5 件のうち 4 件を満たし、条件 2（`geometry` / `calib` / `features` / `objdetect` の追加）は当初計画の側で意図的に次へ送った**（判定表は `docs/roadmap.md` の M5 節）。

**その「次」に着手し、条件 2 は「部分的に満たした」まで進んだ**（2026-09-01、ブランチ `feat/m5-modules`）。`objdetect`（QR コードの符号化・復号、`ocvu_qr_encode` / `ocvu_qr_decode`）と `features`（ORB 特徴点検出、`ocvu_orb_detect`）の 2 module を新設し、C ABI は 20 → 23 本（allowlist は 11 → 14 本）、C# の P/Invoke 宣言は 25 本になった。**続けて `geometry` も出した**（`ocvu_find_homography`。C ABI は 24 本、allowlist は 15 本）。**残るは `calib` だけである。** この作業で分かったのは、`geometry` は**リンクが安い**ということである。復元済みの OpenCV ツリーには 7 つのライブラリが在り、`flann` と `geometry` は他 module の依存として**既にビルドされている**（`COMPONENTS` に足すだけでよい）。**対して `calib` だけが高い** —— `tools/opencv-config.psd1` の `Modules` に無いため、足すと構成ハッシュが変わって 5 platform 分の OpenCV を作り直すことになる（実測: `4785d98e9aad` → `09fcbe260d87`。**この値は 2026-09-01 に測り直した** —— それまで書いてあった `a197bbcbdaf5` は `geometry` を足したときの値で、`calib` のものではなかった）。その後 `geometry` も出したので、**条件 2 は「4 module のうち 3 つを出し、残るは `calib` だけ」という状態である。** `calib` が高いのは変わらない（`Modules` に無いので構成ハッシュが変わり、5 platform 分の OpenCV を作り直す）。

**続けてカメラの歪み補正を出し**（2026-09-02、`ocvu_undistort` / `ocvu_find_chessboard_corners`）、**同じ日に `calib` module を足して校正の輪を閉じた**（`ocvu_calibrate_camera`。ブランチ `feat/m5-calib-module`）。**本数は `docs/api-map.md` の冒頭が数える**（allowlist は `docs/abi-ownership-and-versioning.md` §3）—— ここに写すと、次に 1 本増えたときこの行だけが嘘になる。**これで M5 完了条件 2 の 4 module（`geometry` / `calib` / `features` / `objdetect`）がすべて出た** —— 判定は「満たした」に変わった（詳細は `docs/roadmap.md` の M5 節）。
...
**出していないものを明記しておく。** `calib` には `stereoCalibrate`（ステレオ校正）・`calibrateHandEye`・魚眼系がある。**出したのは単眼カメラの校正 1 本である。**（**ここに挙げていた `geometry` の `solvePnP` は、2026-09 の API 拡張で出した。**）

公開 ABI の内訳は次のとおり（**本数を数えるのは `docs/api-map.md` の冒頭だけ**。ここは何が在るかを説明する）。M0/M1 由来の 8 本（`ocvu_get_abi_version`、last-error の取得、status 表の照会、`ocvu_get_opencv_version` / `ocvu_get_build_information`）に、M2 で `Mat` のライフサイクルと buffer 転送の 6 本（`ocvu_mat_create` / `_release` / `_clone` / `_get_info` / `_copy_from_buffer` / `_copy_to_buffer`。`native/src/ocvu_mat_table.cpp`、`native/src/ocvu_mat.cpp`、`native/src/ocvu_mat_buffer.cpp`）、`imgproc` の 3 本（`ocvu_cvt_color` / `ocvu_resize` / `ocvu_gaussian_blur`。`native/src/ocvu_imgproc.cpp`）、L3 のクラッシュ・ハング耐性を実証する `ocvu_debug_crash`（`native/src/ocvu_debug.cpp`）が加わり、M3.5 で `imgcodecs` の 2 本（`ocvu_imencode` / `ocvu_imdecode`。`native/src/ocvu_imgcodecs.cpp`）、続く M5 の module 拡張で `objdetect` の 2 本（`ocvu_qr_encode` / `ocvu_qr_decode`。`native/src/ocvu_objdetect.cpp`）と `features` の 1 本（`ocvu_orb_detect`。`native/src/ocvu_features.cpp`）、`geometry` の 1 本（`ocvu_find_homography`。`native/src/ocvu_geometry.cpp`）、続けてカメラの歪み補正の 2 本（`ocvu_undistort` / `ocvu_find_chessboard_corners`。**`imgproc` と `objdetect` にまたがるが、実装は `native/src/ocvu_calibration.cpp` の 1 ファイルにまとめてある** —— `calib` module は使っていない）が加わった。**2026-09 の API 拡張で 26 本足した** —— `geometry` の姿勢 4 本と `ocvu_get_perspective_transform`（`native/src/ocvu_pose.cpp` と `ocvu_geometry.cpp`）、`objdetect` の ArUco 2 本（`native/src/ocvu_aruco.cpp`）、`imgproc` の 8 本（`native/src/ocvu_imgproc_ops.cpp` と `ocvu_imgproc_shape.cpp`）、`core` の基本演算 8 本（`native/src/ocvu_core_ops.cpp`）、`features` の記述子 2 本（`native/src/ocvu_matching.cpp`）、**そして新設した `stereo` module の 1 本**（`native/src/ocvu_stereo.cpp`）である。**`stereo` は 8 つ目のリンク済み module だが、`tools/opencv-config.psd1` の `Modules` は触っていない** —— `calib` が推移的に引くので OpenCV 側は既にビルドしており、`COMPONENTS` に 1 語足すだけで済んだ（構成ハッシュは変わらず、6 platform 分の再ビルドは起きていない）。**M7c で `dnn` を 9 つ目のリンク済み module として足した**（下の M7c の段落参照）。所有権・versioning・API allowlist の正本は `docs/abi-ownership-and-versioning.md`。**M5 で宣言の在り処が変わった** —— `extern "C"` の関数宣言は `native/include/opencv_unity_native.h` から module ごとの `native/include/ocvu/{infra,core,imgproc,imgcodecs,objdetect,features,geometry,calib,stereo,dnn}.h` へ移り、**いずれも生成物になった**。もとのヘッダが今も持つのは `OCVU_STATUS_LIST`・handle と struct の型・`OCVU_*` 定数だけで、**関数宣言はもう 1 本も無い**（10 個を include するだけである。**その include の行だけは生成物ではない** —— `tools/tests/BindingGenerator.Tests.ps1` が spec の module 数との一致を要求する）。**「宣言は `opencv_unity_native.h` に在る」と書いた記述は、この時点で誤りになった。**

**M7c（2026-09-08〜10）で `dnn` を opt-in profile として足した。** `bindings/spec/dnn.json` が `"profile": "dnn"` を宣言する初めての module で、C ABI 4 本（`ocvu_dnn_net_read_onnx` / `_net_release` / `_blob_from_image` / `_net_forward`。`native/src/ocvu_dnn.cpp` と handle table の `native/src/ocvu_dnn_table.cpp`）が加わった。**native binary は既定で `dnn` を含む** —— `tools/opencv-config.psd1` の `Modules` に `dnn` を足し（構成ハッシュが変わり 6 platform 分の OpenCV を作り直した）、`cmake/FindOpenCvUnityDeps.cmake` の `COMPONENTS` にも足したので、配る binary は 1 つのままで `dnn` の関数も必ず入っている。**切るのは C# 側だけである** —— 生成された P/Invoke 宣言は別 assembly（`CvUnity.Interop.Dnn` の `NativeMethodsDnn`、`Runtime/Interop.Dnn/`）へ出て、`defineConstraints: ["OCVU_PROFILE_DNN"]` の下でしかコンパイルされない。**利用者が opt-in する経路は Player Settings の Scripting Define Symbols に `OCVU_PROFILE_DNN` を足すことで**、`README.md` / `README.ja.md` に書いてある。**`OCVU_ABI_VERSION` は 1 のままである** —— profile は「どの宣言がコンパイルされるか」を変えるだけで、境界の契約の版は変えない（`docs/abi-ownership-and-versioning.md` §4）。**dnn を足したことで全部入り tarball が配布上限（100 MB）を実際に超えた**（PR #72、`package が上限を超えた: 124101923 > 104857600`）が、**原因は dnn ではなく Android の `.so` に前から積もっていた DWARF デバッグ情報**で、`native/CMakeLists.txt` に Android 限定の strip を足して解消した（詳細と実測は `docs/roadmap.md` の「dnn の配布の形」）。**M7 完了条件 1（profile ごとの native artifact、manifest、third-party notices）の判定は `docs/roadmap.md` の `### M7c の判定` にある** —— 実機で動かしたことは無く、third-party のライセンス面にも未解決の穴が残る。
...
| `./tools/dev.ps1 generate` | **M5。** `bindings/spec/*.json` から C ヘッダ・C# の P/Invoke・到達性テスト・API 対応表を書き出す（**23 ファイル**。M7c で `dnn` が加わり 20 → 23 になった —— ヘッダ 9→10、`NativeMethods.*.g.cs` 9→10、到達性テスト 1→2、`docs/api-map.md` は 1 のまま）。**境界の宣言を手で書く経路はもう無い** | — |
| `./tools/dev.ps1 verify-generated` | **M5。** 生成物が spec と一致しているかだけを見る（書き直さない）。食い違えば差分を並べて非 0 で返る。`test` に入っている | — |
| `./tools/dev.ps1 test` | **既定**。tools の速いテスト + `verify-generated` + L1 + L3 | **約 1 分 32 秒**（増分、成果物が最新。2026-09-07 実測、exit 0。同じ条件で 2 回測って 93.3 秒 / 91.9 秒）。**M3.5 以前の「約 21 秒」→ 2026-09-01 の「約 65 秒」→ 2026-09-06 の「約 2 分 24 秒」→ 2026-09-07 前半の「約 2 分 13 秒」（`dotnet build` を伴う検査を slow lane へ切り出しただけの値）から、この行の値まで縮んだ。** 2026-09-06 の回帰は 2 つの独立な原因が重なっていた: (1) `BindingGenerator.Tests.ps1` の `dotnet build` 検査（単体 36 秒）—— `tools/tests/NonStandardProfileCompile.Tests.ps1` へ切り出し `$ToolsTestScriptsSlow`（CI 専用）へ移した。(2) **こちらが主因だった**: 同じファイルの負の対照が `native/include/ocvu/*.h` 等の生成物をバイト単位で元へ戻しつつ、**その `LastWriteTime` は戻していなかった**。CMake が configure し直した `.vcxproj` を MSBuild が見ると、依存先ヘッダの mtime が新しいというだけで `native/src/*.cpp`（20 本超）全部を再コンパイル対象と判定する。実測: 直したことで `test-tools` 直後の `test-native` 単体が 56 秒 → 9 秒に戻った。**復元は `verify-generated`（spec からの独立な再生成と比較）が成功したときだけ mtime を戻す**——自分が書いた変数を自分で読み比べる形は同語反復（渡す変数を取り違えても常に一致する）になるので採らなかった。負の対照（わざと違う内容で「復元」し、mtime に触れないことを見る）も実測済み |
| `./tools/dev.ps1 test-tools` | `tools/tests/` の速いレーン（OpenCV 構成・ハッシュ無効化・**生成物と spec の一致**・**Unity と Emscripten の対応表**・**tarball の大きさ**・**binary の公開面が spec と一致すること（M7b）**・**benchmark の結果が publish できる形かどうか**）。**本数を写さない。正本は `tools/dev.ps1` の `$ToolsTestScriptsFast` である**（M7 で 2 本増えた。この列挙もそれに合わせて足した——列挙自体を数の代わりにしない） | 約 18 秒（**2 本だった頃の値**）。**2026-09-07 実測では約 75 秒**（74.3 秒 / 74.6 秒 / 75.4 秒、3 回測って安定）——`dotnet build` を伴う検査を抜いたあと、`BindingGenerator.Tests.ps1` の負の対照に mtime 復元の正負両対照を足した後の値。**時間そのものより、この直後に native をビルドしても再コンパイルを誘発しないことのほうが効いている**（下の `test` 行） |
| `./tools/dev.ps1 test-tools-slow` | **CI 専用**。allowlist 検証・restore の実 download・linkage 検証・配布物生成 | 約 4 分 15 秒（2026-08-28 実測。M3 で `VerifyArtifactLinkage.Tests.ps1` と `PackageRelease.Tests.ps1` が加わり、M1 時点の約 70 秒から伸びた）。**M7b で `NonStandardProfileCompile.Tests.ps1`（`BindingGenerator.Tests.ps1` から切り出した `dotnet build` 検査）が加わり、約 4 分 34 秒になった**（2026-09-07 実測、exit 0、186 assertion PASS） |
| `./tools/dev.ps1 test-native` | L1 のみ（GoogleTest **234 件**（M7c で `dnn` の L1 が加わり 215 → 234 になった） + CTest **4 件**） | 約 10 秒（**`test-tools` の直後でも同じ**。2026-09-07 実測、9.0 秒 / 8.9 秒 / 8.8 秒。M7b で mtime 復元を直す前は、直前に `test-tools` を走らせると 56〜57 秒に伸びていた——ヘッダの mtime が新しいままだったため） |
| `./tools/dev.ps1 test-managed` | L3 のみ。**solution の全テストプロジェクトを回す** —— `CvUnity.Tests.Managed` が P/Invoke 越しに実物の DLL を叩く **196 件**（M7a で `AllocationTests` が加わり 181 → 185、M7c で `DnnInferenceTests` が加わり 185 → 195、レビューの M6 で forward の OpenCV エラー経路を実証するテストが 1 件加わり 196 になった）、`Ocvu.Generator.Tests` が spec と生成器を見る **116 件**（M5 で新設、106 件。M7b で profile 分岐のテストが加わり 113 件、最終レビューの C-1 で `LibraryName` の写しを見る 3 件が加わり 116 件になった）。**件数を書いてあるのはこの行だけである** —— 他所に写すと、テストを 1 件足した瞬間にそちらだけが嘘になる（M5 で実際に 4 箇所が同時に古くなった） | 約 11 秒（**44 件だけだった頃の値**） |
...
| `./tools/dev.ps1 test-unity-editmode` | L4（Unity EditMode、Mono、**59 件**。`-testCategory '!Graphics'` で Graphics レーン側のテストを除外する） | 約 27 秒（増分。2026-08-28 実測。**Unity 6000.0.82f1・EditMode 10 件のときの値。** その後 M3.5 で 16 件、M4 で 33 件、M5 で 34 件、2026-09 の API 拡張で 48 件、M7a で 53 件、M7b で `ProfileGatingTests`（4 件）が加わり 57 件になったが所要時間は取り直していない。**59 件は 2026-09-10 に実測した値**（`unity-editmode.xml` の `testcasecount="59"`）。**2026-09-11 に一度 58 件へ減らして戻した** —— `CiVisibilityTests` を消したが、**削除の条件を読み違えていた**（同 class の docstring は「graphics レーンを **CI に配線できたら**」と書いており、実際に必要なのは「**merge を止めるようになったら**」だった。`Unity Graphics (Linux)` は非必須なので、`[Category("Graphics")]` を付ける行為はいまも「必須レーンから、止めないレーンへ移す」ままである）。旧「58 件」は M7 squash（`be5615f`）の時点では正しかった——`tests/UnityProject/Assets/Tests/EditMode/*.cs` の `[Test]`/`[UnityTest]` 属性行を数えると `be5615f` で 65 件、うち Graphics カテゴリ 7 件を除いて 58 件。差の 1 件は M7c（`e1b0930`、"dnn を opt-in profile として足す"）が `ProfileGatingTests.cs` に足した `TheDnnPublicApiAssemblyIsAlsoAbsentWithoutItsDefine`（同ファイルの `[Test]` 数が 5→6）で、同じ数え方で `e1b0930` は 66 件、Graphics 7 件を除いて 59 件になる——この行を更新する commit がその後に続かなかっただけである） |
| `./tools/dev.ps1 test-unity-player` | L5（Unity IL2CPP Player のビルドと実行、**39 件**。`-nographics` で走るので GPU に依る経路は含まない。**M7a で `RenderTextureChecks` の PlayMode 側の対（`RenderTexturePlayerTests`）を足し、35 → 39 になった**） | 約 54 秒（増分・キャッシュ温状態。2026-08-28 実測。cold 実測はまだ無く、roadmap の想定は 5〜20 分。**Unity 6000.0.82f1・10 件での値**） |
| `./tools/dev.ps1 test-unity-graphics` | **M7a。**`RenderTexture` を使う GPU 依存の経路（`GraphicsTests` / `GraphicsBenchmarkRunner`）を EditMode で検証する。`-nographics` を**付けない**（`-nographics` の下では `graphicsDeviceType` が `Null` になり `RenderTexture.Create()` が true を返しつつ画素が読めなくなるため、実測）。**2026-09-11 から `ci-unity.yml` にも同じものが在る**（`Graphics` レーン）—— それまで「ローカル専用」で、このレーンが見るものは CI から完全に見えなかった。**成立の根拠は、game-ci の Editor が `-nographics` で走っていないことである**（コンテナの `unity-editor` が `xvfb-run` から起動する）。実測 run 34612557397 で 7 passed。**CI と `dev.ps1` で起動の仕方は違うが、`-testCategory` の値は機械が突き合わせる** —— `tools/tests/OpenCvConfig.Tests.ps1` が「否定形を使う行は 1 本残らず正本と一致」「肯定形はちょうど 1 本で、その補集合」を見る | 未計測 |
| `./tools/dev.ps1 test-unity-tarball` | **UPM tarball の導入検証。** tarball だけを指した使い捨ての Unity プロジェクトを作り、そこで EditMode を走らせる。**2026-09-11 から CI にも在る** —— `ci-unity.yml` の `tarball` job が `-PrepareOnly -WorkDir` でプロジェクトの準備だけをこのレーンに任せ、Unity の起動を game-ci に渡す（**作り方を CI に書き写さない**。分けると「導入を確かめた tarball」と「配る tarball」が別物になる）。`-PrepareOnly` は native をビルドし直さず（host の新しい glibc で上書きすると v0.1.0 の欠陥を再現する）、Unity も探さない（runner には無い）。`manifest.json` の `file:` 参照は**相対パス**で書く —— コンテナではワークスペースが別のパスに mount されるので絶対パスは解決できない。**ローカルも同じ形にする**（分けると CI で通る形を一度も試さないままになる）。`-testCategory '!Graphics'` で `test-unity-graphics` 側のテストを除外する（`Test-UnityEditMode` と同じ `$script:UnityGraphicsExclusionCategory` を使う——2026-09-10 まで直書きのコピーで抜けており、GPU に依るテストが「スキップ」ではなく「失敗」していた）。**`-PluginSource` に他 platform の plugin 木（';' 区切り）を渡すと全部入りとして固めてから検証する** —— 渡さなければ従来どおり 1 platform 分で走り、**黙って全部入りのふりはしない** | 約 3 分（2026-08-28 実測。Library/ を持って行かないので Unity が import からやり直す。**Unity 6000.0.82f1・1 platform でのもの**）。**6 platform の全部入りは 2026-09-03 に実測** —— `release.yml` の空撃ちが作った実物の tarball を展開して材料にし、`native plugins present: 6` / `34 passed` / exit 0（**所要時間は取っていない**）。**2026-09-10 に Graphics カテゴリの除外を足した後で実測し直した** —— 除外前は `66 passed=60 failed=6`、除外後は `59 passed` で exit 0（`unity-tarball.xml` の `testcasecount`）|
| `./tools/dev.ps1 test-unity-web` | **M6。** WebGL の Player を建て、**ブラウザで走らせる**（Playwright + headless Chromium）。先に `assert-emscripten-version.ps1` が **Unity 同梱の Emscripten を対応表と突き合わせる** —— **このレーンに置くのが正しい**（WebGL の Player を建てられる時点で Unity と WebGL 支援は必ず在るので、「道具が無いから飛ばす」経路が構造的に生まれない）。**stripping は有効のまま**にする（無効にすると到達性テストが確かめたいことを確かめられない） | **所要時間は取っていない。** 通した実績はある（ローカル・CI とも `failed=0`）。**`passed` も `reachable` も数を写さない** —— `passed` は共有本体 `AbiSurfaceChecks` の検査数（`tools/run-web-e2e.ps1` が数えて**完全一致**を要求する）、`reachable` は spec の entry 数で決まる。**写すと、検査や関数を 1 つ足した日にこの行だけが嘘になる**（実際 8 -> 21 で踏んだ） |
| `./tools/dev.ps1 benchmark` | **M7a。** `test-unity-player` と `test-unity-graphics` を順に走らせ、両方の結果 XML から `OCVU_BENCH:` 行（`TestContext.WriteLine` が結果 XML の `<output>` に入れたもの）を集めて `artifacts/benchmarks/latest.json` へ書く。**時間を判定しない** —— 落ちるのは「測れなかったとき」（0（単位は entry ごと。key が `_ns` で終わればナノ秒））だけで、遅い・速いでは落ちない（設計 D1）。数字の読み方は [性能](docs/performance.md) にまとめてある。**2026-09-11 から CI がこれを publish する** —— `ci-unity.yml` の `benchmarks` job が `Standalone` と `Graphics` の結果 XML を集めて `run-benchmarks.ps1` に渡し、`latest.json` を artifact にする（**GPU に依る側が CI で走るようになって初めて成立した** —— `GraphicsBenchmarkRunner` は `Graphics` レーンにしか居ない）。**Unity がある唯一のこのマシンでは、このコマンドは依然として完走しない** —— 内部で呼ぶ `test-unity-player` が後始末段階（`Stop-UnityTestPlayers` 内の `Get-CimInstance`）でハングする既知の欠陥を踏むため（詳細は `docs/roadmap.md` の M7a の判定「穴を隠さず書く」の項）。数字は、ハングしたプロセスを止めたうえで `run-benchmarks.ps1` を直接叩いて得た | 未計測（内部で呼ぶ 2 レーンの合計） |
...
「ローカルループは秒単位を死守する」という不変条件（本ファイル下部）と、`test` の実測（2026-09-07 で約 1 分 32 秒）はすでに緊張関係にある。M1 ではこれを**受け入れて記録するに留めており、解消していない**。**M6 でさらに 4 本目の `pwsh -NoProfile` 起動が加わったので、65 秒はもう上限ではない**（M3.5 のときと同じく、**前提が変わったことは記録し、再計測はしていない**）。**M5 でこの緊張は 1 段強まった** —— M1 の 65 秒は**ソースを変えた**ときの値だったが、2026-09-01 の 65 秒は**何も変えていない**ときの値である（tools のテストが 3 本になり、`verify-generated` と L3 の 2 つ目の assembly で `dotnet` の起動が 2 回増えた）。**M7b で 2 分 24 秒まで悪化したが、原因は 2 つ重なっていた**: `dotnet build` を伴う高い検査（slow lane へ切り出し、2 分 13 秒に）と、**生成物の負の対照がヘッダの mtime を戻していなかったこと**（`verify-generated` の判定で mtime を戻すよう直し、1 分 32 秒に）。後者のほうが影響が大きかった —— `test-tools` 単体の所要時間はほぼ変わっていない（約 69 秒 → 約 75 秒、負の対照を 1 組増やした分だけ**伸びた**）のに、`test` 全体は 41 秒縮んだ。理由は時間そのものではなく、**mtime の副作用が消えたことで native の全 .cpp 再ビルドが起きなくなった**ため（`test-native` 単体 56〜57 秒 → 9 秒）。**65 秒には依然として戻っていない** —— 残る差は `test-tools` が 7 本のスクリプトを抱えるようになったこと自体（2026-08-28 時点の「2 本で 18 秒」から積み上がった、正当な検査の増加）で、着手するなら configure の結果を跨いで再利用するか、OpenCV に依存しないレーンを分けることになる。
...
| 場所 | 内容 |
| --- | --- |
| `bindings/spec/*.json` | **境界の正本（M5）。** module ごとに 1 ファイル（`infra` / `core` / `imgproc` / `imgcodecs` / `objdetect` / `features` / `geometry` / `calib` / `stereo` / `dnn`）。`dnn` は `"profile": "dnn"` を持つ初めての module（M7c）。`schema.json` が形を縛る。**entry 数を数えるのは `docs/api-map.md` の冒頭だけ**である。**ここに 1 エントリ書いて `dev.ps1 generate` を実行するのが、関数を足す唯一の経路である** |
...
| `native/include/opencv_unity_native.h` | 公開 C ABI ヘッダの入口。`OCVU_STATUS_LIST`（status code の唯一の定義元）・handle と struct の型・`OCVU_*` 定数を持ち、下の 10 個を include する。**M5 以降、関数宣言はここに 1 本も無い**（足しても次の `generate` で消える） |
| `native/include/ocvu/{infra,core,imgproc,imgcodecs,objdetect,features,geometry,calib,stereo,dnn}.h` | **生成物（M5）。** module ごとの `extern "C"` 宣言。**手で編集しない** —— `dev.ps1 verify-generated` が落とす |
| `docs/api-map.md` | **生成物（M5）。** spec の entry を 1 行ずつ並べた API 対応表。**本数を数えるのはこの表の冒頭だけ**で、他所に数字を写さない。「OpenCV 全対応」のような曖昧な表現を置かないための出口でもある |
| `native/src/` | C ABI 実装（version / last-error / status 表 / debug throw・crash / OpenCV version・build information / `Mat` のライフサイクルと buffer 転送 / imgproc 3 関数 / **imgcodecs の encode・decode 2 関数** / objdetect の QR 符号化・復号 2 関数 / features の ORB 検出 1 関数 / geometry の射影変換推定 1 関数 / **カメラ校正 3 関数（`native/src/ocvu_calibration.cpp`。`objdetect` の `ocvu_find_chessboard_corners`・`calib` の `ocvu_calibrate_camera`・`imgproc` の `ocvu_undistort` が 1 ファイルに同居する ── 用途は 1 つだが 3 module にまたがるため）**、**2026-09 の API 拡張の 7 ファイル（`ocvu_pose.cpp` 姿勢 4 本 / `ocvu_aruco.cpp` ArUco 2 本 / `ocvu_imgproc_ops.cpp` 画素を作る 5 本 / `ocvu_imgproc_shape.cpp` 形を返す 3 本 / `ocvu_core_ops.cpp` 基本演算 8 本 / `ocvu_matching.cpp` 記述子 2 本 / `ocvu_stereo.cpp` 視差 1 本）**、**dnn の 4 関数（M7c。`ocvu_dnn.cpp` に ONNX 読み込み・blob 化・forward、handle table は `ocvu_dnn_table.cpp` に分けてある）**） |
| `native/tests/` | L1 の GoogleTest と、意図的にクラッシュ・ハングする `ocvu_probe` |
| `cmake/run_expect_failure.cmake` | 「失敗するはずのコマンド」を走らせる CTest ドライバ |
| `cmake/FindOpenCvUnityDeps.cmake` | `third_party/opencv/<hash>/` を探して OpenCV を取り込む。**`COMPONENTS` の行がこのプラグインの実際のリンク対象を決める**（現在は `core imgproc imgcodecs objdetect features geometry calib stereo dnn`。**`flann` と `geometry` は `features` / `objdetect` の依存として推移的に引かれるので、書かなくてもリンク行に入る** —— 明示は意図の宣言であって、no-op であることは実測で確かめてある。**対して `calib` は違う** —— どの module からも引かれないので、この 1 行が実際にリンク行を変える。足す前は `cv::calibrateCamera` が未解決の外部シンボルになった）—— `tools/opencv-config.psd1` の `Modules` は OpenCV 側が何をビルドするかであって、こちらが何をリンクするかではない |
| `native/modules.cmake` | **正本（M7b）。** `OCVU_MODULES` に渡した module の一覧から `OCVU_SOURCES` を組み立てる —— **このプラグイン自身が何をビルドするか**を決める。**同じ「module」という言葉が指す 3 つは別物である**: `tools/opencv-config.psd1` の `Modules` は OpenCV 側が何をビルドするか、`cmake/FindOpenCvUnityDeps.cmake` の `COMPONENTS` はこのプラグインが何をリンクするか、この `OCVU_MODULES` はこのプラグインが何をビルドするか。既定は `OCVU_ALL_MODULES`（全 10 module。M7c で `dnn` が加わった）。`infra` と `core` は他の全 module が使うので外せない（`OCVU_REQUIRED_MODULES`。外そうとすると configure が `FATAL_ERROR` で落ちる）。**`-DOCVU_MODULES=...` は CMake キャッシュに sticky である** —— 一度絞ると `-UOCVU_MODULES` で明示的に外すかビルド木を作り直すまで既定へ戻らないので、configure のたびに実際の一覧を `message`（既定でないときは `WARNING`）で可視化する。**CMake target は今も 1 つのまま**である —— ここが組み立てるのは**ソースの一覧**であって、コンパイル済みの中間物ではない。`native/CMakeLists.txt` は同じソースを `opencv_unity_native`（`OCVU_BUILDING_DLL`）と `ocvu_static`（L1 テスト用、`OCVU_STATIC` を PUBLIC で持つ）へ **2 回コンパイルする**ので、1 度だけコンパイルして両方へ配る形は取れない。profile との関係は `docs/abi-ownership-and-versioning.md` §4 が正本 |
| `tools/opencv-config.psd1` | OpenCV ビルド構成の唯一の定義元。**`Modules` は現在 `core imgproc imgcodecs objdetect features calib dnn` の 7 つ**（**`stereo` は入っていない** —— `calib` が推移的に引くので OpenCV 側は既にビルドしており、`COMPONENTS` に足すだけで済んだ。構成ハッシュは変わっていない）（`calib` は 2026-09-02 に、`dnn` は M7c で足した。どちらも構成ハッシュが変わるので、触ると**全 platform 分**の再ビルドになる —— **数を写さない。正本は同ファイルの `Toolchains`**）。`Toolchains`（platform 別 generator/architecture）と `PlatformCMakeArgs`（platform 固有 flag）+ 共通の module allowlist・CMake flags（M3 Task 1 で platform 別化） |
...
| `tools/verify-exported-symbols.ps1` | **binary の公開面が spec と完全一致することの検証（M7b）。** 走るのは `ci-native.yml`（`dev.ps1 test` が作った開発用の binary）と `release.yml`（**配る package に置かれた実物**）の 2 経路で、後者は最終レビューで足した —— それまで「配布 binary の公開面」と書いてありながら、配る経路には 1 度も掛かっていなかった（`verify-plugin-portability.ps1` が M3 で踏んだ形と同じ）。**どちらの経路も desktop 3 platform だけである** —— iOS / Web は静的ライブラリで「export 面」の概念が違い（`verify-static-bundle.ps1` が別に見る）、Android は共有ライブラリだが host の `nm` が arm64 の ELF を読めるかを確かめていないので明示的に skip する。 Windows は `dumpbin /EXPORTS`、Linux は `nm -D`（動的シンボル表）、macOS は `nm -g`（Mach-O には `.dynsym` に相当する別表が無いのでこちらで足りる）で実物の export を読み、`ocvu_` で絞ったうえで spec の関数名（`entryPoint` を持つ entry は畳む）と過不足なく突き合わせる。**両向きに効く** —— spec にあるのに export されていない関数も、export されているのに spec に無い関数も検出する。**`ocvu_` で絞る理由は実測で裏づいている**——3 platform とも `ocvu_` の export が spec の関数一覧と過不足なく一致した一方（**本数を写さない。正本は `docs/api-map.md` の冒頭である** —— 写していた「53 本」は M7c の `dnn` が入った時点で古くなった）、絞りを外すと Linux で 3 件（`cv::cornerScore<N>` のテンプレート実体化）、macOS で 1895 件の非 `ocvu_` export が出た（**macOS の 1895 は `nm -g` という道具の性質かもしれない** —— 実際の動的 export 面がもっと小さい可能性は測っていない）。`native/modules.cmake` の `OCVU_MODULES` を絞ると、外した module の関数が実際にこの検査で「不足」として捕まる |
...
| `tools/tests/OpenCvConfig.Tests.ps1` | 構成ハッシュの検査（`dev.ps1 test` に入る速いレーン）に加えて、**workflow 側の不変条件を job 単位で**見る: `container:` を持つ job の宣言イメージが `opencv-config.psd1` と一致すること、その job の `run:` に `sudo` が残っていないこと、`opencv.ps1 restore` を呼ぶ job が `third_party/opencv/` を対象にした cache step を**ちょうど 1 つ**持ち、その `path:` と `key:` の両方が構成ハッシュを参照し、参照先の step id が同じ job に実在すること、テストレーンを走らせる job が結果を `if: always()` で artifact 化すること、全 job に `timeout-minutes` があること、配る binary を作る job が step 単位の `if:` 無しで移植性を検査し、その振り分けが matrix の platform 集合と過不足なく一致すること。**Unity を起動する job については**（`ci-unity` の中で `game-ci/unity-test-runner` を使う job だけに絞る。絞った先が空でないことも見る）、**全 platform を重ねる step**・合図を置く step・`assert-unity-results.ps1` に `-RequireTest` を渡す step がそれぞれちょうど 1 つあること、`needs:` を持つなら**ちょうど `if: !cancelled()`** で守られていること（ゆるい条件は必須チェックを**常時** skip にしうる）、合図の名前が C# の定数・workflow・`dev.ps1`・`.gitignore` の 4 箇所で一致していること。**job 単位にしたのは、ファイル単位では通ってしまう穴を実際に踏んだから**である——cache の検査が `ci-native` の Windows job の分で満足し、後から足した macOS / Linux と `ci-sanitizers` の linux-asan が漏れていた。**コンテナ名の検査も同じ穴を持っていた**: 4 本の workflow を名指ししていたので `nightly` を見落としており、`container:` を持つ全 job を対象にする形に直した。切り出しは YAML パーサを使わずインデント規約で行うが、**切れなかったときは空振りではなく落ちる**（`jobs:` 直下の 2 スペース key を緩く数えた数と、job として認識した数が合わなければ失敗させる）。**ファイル単位で見るもの**は 3 つある: `# shellcheck ` で始まる散文（道具が指示文として読んでしまう）、`tools/tests/*.Tests.ps1` が `dev.ps1` の `$ToolsTestScriptsFast` / `$ToolsTestScriptsSlow` に配線されていること、`.github/codeql/codeql-config.yml` の `query-filters`（下の行）。workflow の列挙は `.yml` と `.yaml` の両方を拾い、`git ls-files` の一覧と突き合わせる——**検査対象から静かに外れる**が構造上ありえないようにするため。**リポジトリ全体で突き合わせるもの**が 2 つある: 全 workflow の `uses:` が固定された版であること（`@main` / `:latest` を禁じる）と、**同じ action が 2 つの版で参照されていないこと**（名前は `owner/repo` に丸めるので、`codeql-action/init` だけ上げて `analyze` を忘れると落ちる）、そして全 `.csproj` の `PackageReference` が同じ版であること。**後ろ 2 つはどちらも実際に踏んだ形である** —— `actions/upload-artifact` が `unity-probe.yml` でだけ v6 のまま残り、**どの workflow も緑のまま**だった（2026-09-01 に発見。unity-probe は手動起動なので、次に誰かが起動するまで走りもしない）。拾えた `uses:` の数が緩い数え方と合わなければ、空振りではなく落とす |
| `tools/tests/EmscriptenVersion.Tests.ps1` | **対応表の自己整合を、速いレーンで見る（M6）。** `tools/emscripten-versions.psd1` に「この Unity の系列に対応する項が在るか」「workflow の pin と一致するか」を見る。**表が現実と合っているかは見ない** —— それは Unity の実物を要る話なので `tools/assert-emscripten-version.ps1` が持つ。**2 つで 1 組である** |
...
| `cmake/toolchains/<platform>.cmake` | クロスビルドの toolchain（M4 で Android NDK / iOS、**M6 で Emscripten**）。**`web-wasm.cmake` は Emscripten 同梱の `Emscripten.cmake` を先に include してから足す**（先に書くと上書きされる）。**platform 固有の linker flag はここに置く** —— `tools/opencv-config.psd1` に書いても OpenCV は共有ライブラリを作らないので何にも当たらない（16 KB page size でこれを踏んだ）|
| `tools/emscripten-versions.psd1` | **Unity の minor 系列 -> その Unity が同梱する Emscripten の版。正本（M6）。** **これは写しである** —— Unity が何を同梱するかはこちらで決められないので、値は必ず「誰かが読んで書き写したもの」になる。**だから写しだけを持たない**: 表の自己整合は `tools/tests/EmscriptenVersion.Tests.ps1`（速いレーン）が、**Unity の実物との突き合わせ**は `tools/assert-emscripten-version.ps1` が見る。**版を合わせる必要があるのは、LLVM がバージョン間のバイナリ互換を保証しないから**である —— CI は Unity を持たないので emsdk から入れるが、そこが食い違うと **CI で通った wasm が Unity のビルドで壊れる** |
| `tools/assert-emscripten-version.ps1` | 上の表を **Unity の実物**と突き合わせる（M6）。**SKIP の経路を作っていない** —— 「道具が無いから飛ばす」が構造的に生まれないよう、WebGL の Player を建てられる場所でだけ呼ぶ。CI では Unity がコンテナの中にしか無いので、**Unity 自身に版を書き出させて `-RecordedVersionPath` で読む**（`BuildPlayer.BuildWebGL`。**書き出しはビルドの後でなければならない** —— 出来る場所が repo root ではなくプロジェクトの下である） |
| `tools/Emscripten.psm1` | Emscripten の在り処を解決する（M6）。**既存の設定を尊重する**（emsdk は `$EMSDK/.emscripten` を書くが `EM_CONFIG` は立てない。Unity 同梱は `FROZEN_CACHE` を持つ）。**配置を決め打ちせず `clang` / `wasm-opt` を探して確かめる** |
| `tools/verify-static-bundle.ps1` | **静的ライブラリに OpenCV が束ねられていることを実証する**（M6）。**`nm` は archive の member ごとに未定義を報告するので「未定義が無い」は誤り** —— **定義の差集合**（未定義のうち、この archive のどこにも定義が無いもの）を見る。**`ocvu_` の側も数える** —— docstring が主張していた保護が実際には無かったとレビューが実測したためで、**この検査が実際に「`.a` を 8 バイトのダミーで上書きした」事故を捕まえた** |
| `tools/verify-wasm-features.ps1` | wasm が要求する機能を**出来た物から読む**（M6。`simd128` が在り `atomics` が無いこと）。**外部の道具に頼らず section を直接読む。** `-RequireInEvery` は「和集合では弱い」を塞ぐ —— archive 全体の和集合ではなく**全 module に要求する**（このブランチが直した回帰を、和集合では捕まえられなかった） |
| `tools/run-web-e2e.ps1` | **ブラウザで Player を走らせて判定する**（M6。Playwright + headless Chromium）。**下限だけを見ない** —— `passed` が**共有本体から数えた件数と完全一致**することを要求するので、検査が消えれば赤くなる。node の静的サーバを自前で建てる（Unity 同梱の `SimpleWebServer.exe` は Windows 専用）。`.gz` を直接要求されたときに `Content-Encoding` を返す必要がある |
| `cmake/bundle_wasm_archive.cmake` | `emar -M` に MRI script を **stdin から**食わせる driver（M6）。`add_custom_command` は shell を通さないのでリダイレクトが書けない。**終了コードだけを見ず、出来た物の存在まで確かめる。** 先例は `cmake/run_expect_failure.cmake` |
...
| `Packages/.../Runtime/UnityIntegration/RenderTextureConverter.cs` | `RenderTexture` から `CvMat` を作る（M7a）。**新しい C ABI 関数は使わない**（既存の `CopyFrom(IntPtr, long, long)` の上に立つ純 C#）。同期の `ToMat` は GPU の転送が済むまで戻らず、`RequestMat`（`AsyncGPUReadback`）は依頼だけして戻る。**`-nographics` では動かない** —— `RenderTexture.Create()` は true を返すが `ReadPixels`/readback の画素が塗った色にならない（実測）。`WebCamTextureConverter` と同じ規約で上下を反転する |
...
| `tools/run-benchmarks.ps1` | Unity の結果 XML から `OCVU_BENCH:` 行を集めて `artifacts/benchmarks/latest.json` へ書く（M7a）。**時間の速い・遅いは判定しない** —— 時間の閾値は共有ランナーの上でフレークになるので、「測れたか」だけを見る: **XML ごとに**（M7a のレビューで直した。連結後に見ると片方のレーンの benchmark 本体が消えても気づけない）1 本以上拾えたか、そして各値が 0（単位は entry ごと。key が `_ns` で終わればナノ秒）でないか（測定が効いていない値を publish しない）。**ログではなく結果 XML の `//output` を読む** —— このリポジトリの確立した型は `TestContext.WriteLine` で、NUnit3 がそれを結果 XML の `<output>` ノードへ入れる |
| `tools/measure-package-size.ps1` | UPM tarball のバイト数を測り、上限（`-MaxBytes`）を超えたら落とす（M7a）。**時間と違ってこれは assert してよい** —— 同じ入力から同じ tarball ができるので CI ランナーの負荷に左右されない。`tools/tests/PackageSize.Tests.ps1` が合成した入力（fake tarball）で発火を確かめ、`release.yml` が全部入りの実物に対して呼ぶ（`-MaxBytes 104857600` = 100 MB。**v0.4.0 の実測 77,530,189 バイトに対して約 1.35 倍** —— 倍増を捕まえつつ、通常の増分では発火しない余裕。**この比は版ごとに縮む** —— 上限を固定したまま中身が増えるので、**余裕が無くなる前に「上限を上げるか、配る形を変えるか」を決めることになる**） |
| `tools/assert-unity-results.ps1` | Unity のテスト結果 XML の判定。**ローカルと CI の両方がここを通る** —— CI では game-ci が Unity を起動するので起動の仕方は分かれるが、判定は分けない。「0 件で緑にしない」もここが持つ。**`-RequireTest` は「その 1 群だけが消えた」を捕まえる**（M3.5 の追補）—— passed が 1 以上なら緑になるので、`PluginGatingTests` が assembly から外れても残りが通れば気づけない。実測: 結果 XML から gating の 6 件を抜くと `10 passed` になり、以前はそれで緑だった。**`Passed` であることまで見る** —— NUnit は `[Ignore]` を `Skipped` として出力し `failed` に数えないので、存在だけを見ると `[Ignore]` 1 行で満たせる（実測）。**`-RequireOutput` はさらに「どの分岐を通ったか」を見る** —— テストが通っても、合図が届かなければ弱い側の分岐を通っただけで、出力は成功時と 1 バイトも違わない。入力（合図を書く step が在ること）をいくら検査しても届いたことの証明にはならないので、テスト自身が出した事実（`native plugins present: 6`）を要求する。**数は写さず `tools/dev.ps1` の `$AllPlatformBinaries` から導く**（M6 のレビューで「検査が古い数を機械で強制していた」ことが分かったため） |
| `tools/pack-upm-tarball.ps1` | UPM tarball を作る唯一の入口。`release.yml` と `dev.ps1 test-unity-tarball` の**両方**がここを通る（作り方が分かれると、導入を確かめた tarball と実際に配る tarball が別物になる）。中身は npm と同じく `package/` の下に入れる — package ID のディレクトリごと固めると UPM が 展開後の root に `package.json` を見つけられず導入に失敗する（M3 で実測）。**M3.5 で `-AllPlatforms` が加わり、これが配る正になった** —— 全 platform 分の binary を 1 つに束ね（**数を写さない。正本は `tools/dev.ps1` の `$AllPlatformBinaries` である**）、名前から版番号を落とす（`com.ayutaz.opencv-unity-native.tgz`）。**存在検査だけにしない**: 全部入りは中身の一覧そのものが契約なので、**全 platform 分が揃っている**ことに加えて**予期しない binary が 1 つでもあれば止め**、固めた後の archive の中を数え直す。`-MaxBytes`（既定 512 MB）を**引数にしてあるのは、既定のままではこの検査が働くところを誰も確かめられないから**である（現在の配布物は上限に対して 1 桁以上小さい） |
| `tools/assemble-plugins.ps1` | 全 platform の plugin 木を 1 つに重ねる（全部入りの材料。M3.5。**数を写さない**）。**何でもコピーしない** —— 既知の binary と `.meta` だけを運び、知らないファイルがあれば止める。binary と `.meta` は必ず対で運ぶ（**binary の無い `.meta` を置くと Unity が消す**。下の `tools/plugin-meta/` の行）。**位置引数を禁止してある** —— `pwsh -File` から空白区切りで 2 つ渡すと 2 つ目が `PackageDir` に入り、**元の木を上書き先と取り違えたまま静かに成功する**（実測で踏んだ）。複数の元は ';' 区切りで受ける |
| `tools/package-release.ps1` | 配布物一式（`checksums.txt` / `sbom.spdx.json` / `build-manifest.json` / `THIRD_PARTY_NOTICES.md` の 4 点）を実物の artifact から生成する（M3 Task 6。手で書かない）。通知はリポジトリ root の全文に、**実物から拾った component 一覧の platform 固有ヘッダを足して**同梱する。**`-ChecksumsOnly` は `checksums.txt` だけを出す**（M3.5、全部入り用）—— SBOM と build-manifest はどちらも「復元済み OpenCV artifact（= 実行中 platform のもの）」から作るので、**全 platform を束ねる job には元が無い**。**統合版を捏造せず、中身の説明は platform ごとの物に任せる** |
...
| `Packages/com.ayutaz.opencv-unity-native/Runtime/Interop/` | P/Invoke の置き場。**宣言は `NativeMethods.<module>.g.cs`（生成物）にあり、手書きの `[DllImport]` は 1 個も無い**（M5。本数は `docs/api-map.md` が数える）。手で残してある `NativeMethods.cs` が持つのは `LibraryName` と、spec が表現しない struct 3 つ（`OcvuMatInfo` / `OcvuKeyPoint` / `OcvuDMatch`）だけである |
| `Packages/com.ayutaz.opencv-unity-native/Runtime/Interop.Dnn/` | **非既定 profile（`dnn`）の宣言先（M7b で機構を作り、M7c で初めて中身が入った）。** spec の `profile` が `dnn` の module があれば、生成物 `NativeMethods.<module>.g.cs` はここへ、クラス名は `NativeMethodsDnn`、assembly は `CvUnity.Interop.Dnn` になる（規約は `docs/abi-ownership-and-versioning.md` §4）。asmdef は `defineConstraints: ["OCVU_PROFILE_DNN"]` を持ち、既定ビルドには含まれない。**M7b の時点では commit された状態で中身が空だった**（`bindings/spec/dnn.json` がまだ無かった）が、**M7c が `bindings/spec/dnn.json` を書いて `NativeMethods.Dnn.g.cs` に実際の 4 関数を出した**——もう空ではない。define を立てて Unity 自身に「実際にコンパイルされ、外すと消えるか」を問う検査は `tests/UnityProject/Assets/Tests/EditMode/ProfileGatingTests.cs` にある。**生成物が実際にコンパイルできることは 2026-09-06 まで誰も確かめていなかった** —— 確かめられていたのは「`defineConstraints` が**手書きの**コードを切る」ことまでで、その穴を通って欠陥が 1 件入っていた（生成物の `[DllImport(LibraryName, ...)]` が `LibraryName` を見られず CS0103。**非既定 profile は別 assembly・別型なので、手書きの `NativeMethods.cs` にある `internal const` が見えない**）。いまは生成器がその 5 行を `#if` 分岐ごと複製し、写し 2 つを `Ocvu.Generator.Tests` の `ProfileTests` が読み比べ、**生成物そのものをコンパイルする検査**が速いレーン（`tools/tests/BindingGenerator.Tests.ps1`）に在る。切り出さなかった理由は `docs/abi-ownership-and-versioning.md` §4 |
| `Packages/com.ayutaz.opencv-unity-native/Runtime/Dnn/` | **`dnn` の C# 公開 API（M7c）。** `CvDnn.cs`（`CvUnity.Dnn` assembly、`CvUnity.Dnn.asmdef` も `defineConstraints: ["OCVU_PROFILE_DNN"]` を持つ）が `NativeMethodsDnn` の上に立つ。`Runtime/UnityIntegration/` と同じく、profile を切った利用者のビルドからは丸ごと消える |
| `Packages/com.ayutaz.opencv-unity-native/` | UPM パッケージ本体（`Runtime/Core`、`Runtime/Interop`、`Runtime/UnityIntegration`）。**`Runtime/Plugins/` は丸ごと成果物で、git は追跡しない**——binary も `.meta` も。`dev.ps1 build` がビルドした platform の binary と `.meta` をここへ置く。**配る正の全部入りでは 全 platform 分（M6 以降は 6 つ）が同居する**（`tools/assemble-plugins.ps1` が重ねる） |
...
| `docs/performance.md` | **M7a。** 測った数字（境界のコピー、`Texture2D`/`RenderTexture` 経路、起動時間）と、測っていないことを書く。**時間は公開するが assert しない**（設計 D1）。`RenderTexture` の同期・非同期 2 経路が run をまたぐと大小が入れ替わる実例を載せ、native texture pointer を実装しないと決めた理由と再評価の条件を書く |
| `docs/api-reference.md` | C ABI と C# 公開 API（`CvMat`/`CvOps`/`CvCodecs`/`CvQrCode`/`CvFeatures`/`CvGeometry`/`CvCalibration`/**`CvAruco`**/**`CvCoreOps`**/**`CvStereo`**/`CvNative`/`TextureConverter`/`WebCamTextureConverter`/**`RenderTextureConverter`**/`NativeArrayExtensions`/**`CvDnn`**/**`CvNet`**）のリファレンス（M3 Task 7。M3.5 で `ocvu_imencode` / `ocvu_imdecode` と `CvCodecs` が加わった。M5 で `CvCalibration` が加わった。M7a で `RenderTextureConverter` が加わった。M7c で `CvDnn` / `CvNet`（§2.17）が加わった） |
...
| `.github/dependabot.yml` | `actions/*` の可変タグと `tests/Managed` の NuGet を週 1 で追う。**「いつの間にか変わっていた」を「差分として見える変更」に変える**のが目的で、上流が壊れた版を出したときにこちらは何も変えていないのに壊れる、という経路を塞ぐ。M3 後に追加。**bot の PR はそのままでは merge できない** —— GitHub は Dependabot が起こした実行にリポジトリの secret を渡さないので、必須チェックである Unity の 2 レーンが `Missing Unity License File` で必ず落ちる（実測: PR #49 は他の 25 チェックが全部緑で、この 2 本だけが赤かった）。**だから扱いは「bot の差分を人が出す PR に取り込み、bot の PR は閉じる」である** —— #18〜#26 の 9 件がその形で閉じてある。**`docker://` で始まる `uses:` は Dependabot が構造的に見ない**（parser が読み飛ばす）ので、`ci-lint.yml` の actionlint だけは手で上げる。理由と手順はこのファイルの冒頭コメントにある |
...
| --- | --- | --- | --- |
| `ci-native.yml` | push(main) / PR / 手動 | `dev.ps1 test` を 3 platform で（**L0 + L1 + L3**。M5 以降、生成物と spec の一致検査もここに乗る）。**desktop 3 job とも `test-tools-slow` を配線してある。**クロスの 3 job（Android / iOS / **Web**）はビルドのみ | **3 platform とも green**（M3 の PR #8 以降） |
| `ci-sanitizers.yml` | push(main) / PR / 手動 | L2。Windows は ASan、Linux は ASan+LSan（リーク検出はこのレーンだけが担う） | green |
| `build-opencv.yml` | 手動 + 構成ファイルの変更 | allowlist 構成の OpenCV を**全 platform**でビルドし artifact 公開（**数を写さない。正本は `tools/opencv-config.psd1` の `Toolchains`**） | 稼働中 |
| `ci-unity.yml` | push(main) / PR / 手動 | L4（EditMode）+ L5（IL2CPP Player）。**Unity は ubuntu で走る**が、**2026-08-30 から他 platform の plugin もビルドし、Unity job がそれを重ねてから走る**（**現在は** `windows-2022` / `macos-14` を含む plugin の 5 job で、全部入りの gating を**要素 6 個**の集合に当てる。**M4 の直後は 4 job / 要素 5 個で、M6 で 1 つずつ増えた**）。**2026-09-11 に Unity のレーンが 2 本から 5 本になり、job も 2 つ増えた** —— matrix に `Graphics`（`-testCategory Graphics`。GPU に依る経路。**game-ci の Editor が `-nographics` で走っていないので成立する**）、`DnnEditMode` / `DnnStandalone`（`ProjectSettings.asset` に `OCVU_PROFILE_DNN` を書いてから走らせる）が加わり、job としては `tarball`（配る全部入りを使い捨てプロジェクトへ導入する）と `benchmarks`（`Standalone` と `Graphics` の結果から `latest.json` を publish する）が加わった。**M6 で 3 つ目の job `Web browser E2E` が加わった** —— WebGL の Player を建て、**Linux の headless Chromium で実際に走らせる**（Unity Test Framework は WebGL の Player を batchmode から走らせられないので、Player 自身に検証本体を走らせて結果をブラウザのコンソールへ出し、それを外から読む）（Windows で走らせない理由として記録していたものは 2026-08-29 に崩れた。上記参照）。したがって CI の L5 は Linux の IL2CPP Player で、Windows 版はローカルのレーンが担う。Unity の導入とアクティベーションは game-ci、合否の判定は `tools/assert-unity-results.ps1`（ローカルと共通）。Unity の版は `ProjectVersion.txt` から取るので、載せ替えても workflow は変えなくてよい。**`game-ci/unity-test-runner` は `v5.0.0-beta.1` の commit を指している**（2026-09-01。タグではなく SHA —— pre-release のタグは上流が消せる）—— 安定版はどれも `node20` を宣言しており、**この repo で node20 を宣言する action はこれだけだった**。`action.yml` の差は `node20` -> `node24` の 1 行だけで、入力 24 個・出力 2 個は名前も既定値も同一である（実測）。**ただしそれはインタフェースの話で、挙動の証明ではない** —— 中身の `dist/index.js` は実装が game-ci/cli へ移ったことで大きく入れ替わっている。**pre-release を必須チェックに入れている**ことは自覚しておくこと（上流は「main CI ではなく別 workflow で試すのが安全」と書いている）。安定版が出たら差し替える。**beta に切り替えたのは 2026-09-01（PR #57）で、それ以降の green は beta のものである** —— M6 の PR #63（2026-09-03）以降に merge された PR がすべてそれに当たる。**以前ここには PR 番号を列挙していたが、やめた** —— 「この列挙は網羅を主張しているので、PR を 1 本通すたびに伸ばすか、やめるかのどちらかである」と自分で書きながら、**実際に 2 度置いていかれた**（#69 で止まり、直した後また #73 で止まった）。**伸ばし続けられなかったので、やめた側を選んだ。****それより前の green（M3.5 / M4 / M5 の 4 件）は `@v4` のものである** | **green**（2026-08-29 に初めて。main の先端 `68fbdae` でも green）。**ただしM3.5 で Unity を 6000.3.16f1 へ、EditMode を 16 件へ替えた後も green**（run の詳細は `docs/roadmap.md` の M3.5 節。**同じ事実を 2 箇所に書かない**）。**2026-08-30 に 3 platform 構成でも green**（PR #37、run 33290375806。`native plugins present: 3` と gating 4 件の個別 Passed を CI が出した）。**M4 で 5 platform 構成になった後も green**（PR #41。`native plugins present: 5`、EditMode 33 passed / Standalone 18 passed）。**M5 で到達性テストが 1 件加わった後も green**（PR #55、2026-09-01。`==> [EditMode] 34 passed` / `==> [Standalone] 19 passed` で**ローカルの実測と一致**。Standalone は stripping 済みの実物で、生成した P/Invoke が全部解決した）。**M6 で Web が加わった後も green**（PR #63、2026-09-03。`native plugins present: 6` と gating 4 件、そして新しい `Web browser E2E` が `passed=8 failed=0 reachable=28`。**この時点では EditMode / Standalone の件数は M5 から変わっていなかった** —— M6 は platform を足したのであって検証本体を足したのではない）。**2026-09 の API 拡張で 26 本を足した後も green**（PR #69、2026-09-05、run 33945388389。`==> [EditMode] 48 passed` / `==> [Standalone] 33 passed` / `native plugins present: 6` / `Web browser E2E` が `passed=21 failed=0 reachable=54`。**3 レーンとも件数が動いた最初の変更である** —— 公開 API を Unity の中で動かす検査を共有本体に 13 件足したので、**手書きで配線する EditMode / PlayMode と、リフレクションで拾う Web が同時に増えた**）|
| `ci-lint.yml` | push(main) / PR / 手動 | 4 job: actionlint（workflow の構文・式・`run:` の中の shell。**版は `1.7.12` に固定してある** —— `:latest` だと上流が規則を足した日に、差分が無いまま必須チェックが赤くなる）/ shellcheck（hook と CI のスクリプト）/ PSScriptAnalyzer（`tools/` の PowerShell、Error のみ）/ 文書の相対リンク検査（コードブロックの中は見ない。0 件なら「壊れていない」ではなく走査が効いていないとして落とす）。**静的に読めば分かる誤りを CI 1 周（10〜20 分）かけて確かめていた**のを埋める。数分で終わるので重いレーンより先に落ちる | green |
...
**このうち後ろ 2 つは、逆向きに解消した。** iOS は M4 で、Web は M6 で **外さずに実装した**ので、「スコープから外れる」は起きにくくなった。**generator も M5 で採用した**ので、「bridge を大量に手書きする」も起きない。**残っている現実的なトリガーは Rust の sanitizer だけである。**

再評価を安価に保つため、**public C header と契約テスト（L1 / L3）は backend 実装から独立に保つ**。この不変条件は M0 で確立し、以降のすべてのマイルストーンで維持する。

## マイルストーン（現在地: **M7（Optional profiles と性能）は完了条件 5 件すべてを満たした**（2026-09-10）。**M7 が最後のマイルストーンである** —— `docs/roadmap.md` の `## M` 見出しは M0 から M7 までで、M8 は無い。**判定が 1 節にまとまっていないのは M7 だけで、5 件が 4 節（`M7a の判定` / `M7b の判定` / `CUDA / cuDNN 同梱の判定` / `M7c の判定`）に分かれている** —— 索引は同文書の `M7 の判定`。**「満たした」と「実証した」は M7 では大きく食い違う** —— `dnn` は実機で 1 度も動かしておらず、`RenderTexture` の画素を運ぶ経路は CI が 1 度も実行せず、CUDA / cuDNN のライセンス条項は読んでいない。**穴の一覧は roadmap の `M7 の判定` にある。ここに写さない。** **M7 の成果は v0.4.0（2026-09-12 公開）で初めて利用者に届いた**（最新の公開版は v0.4.0）。その前の **M6（Web / Wasm）も完了条件 5 件すべてを満たした**（2026-09-03、PR #63 → `1e68d27`）—— **Web が 6 つ目の platform になった**（判定表は `docs/roadmap.md` の M6 節）。**M6 が足した 3 本の check を必須にするかは下の「機構として強制されていること」が持つ**（**ここには書かない** —— 昇格した日に、写した側だけが嘘になる）。**Web にだけ在る制限が 1 つある: `imgcodecs` は JPEG のみで PNG を持たない**（Unity の WebGL 支援が自前の libpng を同梱しているため。他の 5 platform は両方持つ）。その前の **M5（binding specification と generator）も完了条件 5 件すべてを満たした**（2026-09-02）。生成の仕組みは `docs/superpowers/plans/2026-08-31-m5-binding-generator.md` で成立し、**条件 2（`geometry` / `calib` / `features` / `objdetect` の追加）は 3 つの続きの計画で閉じた** —— `objdetect` / `features`（`.superpowers/sdd/2026-09-01-m5-modules-objdetect-features/`）、`geometry`（同上の続き。**リンクは無料だった** —— `features` / `objdetect` の依存として推移的に引かれており `COMPONENTS` に足す必要すら無かった）、カメラの歪み補正（`docs/superpowers/plans/2026-09-01-m5-calib-undistort.md`）、そして **`calib` module と `cv::calibrateCamera`**（`docs/superpowers/plans/2026-09-02-m5-calib-camera.md`）。**最後の 1 つだけが高かった** —— 構成ハッシュが `4785d98e9aad` → `09fcbe260d87` に変わり、5 platform 分の OpenCV を作り直した。**本数は `docs/api-map.md` の冒頭が数える。****M4（Mobile）も完了していない** —— 完了条件 9 件のうち **3 件**が閉じていない（iOS 実機 / lifecycle / macOS 上の Unity）。**Windows IL2CPP は「結論を出す」ことが条件だったので満たしている**（2026-08-31 に実測して「CI で回さない」と結論した）。判定表は roadmap の M4 節が正本。**v0.3.0 を 2026-09-04 に公開した。** **M4（5 platform）・M5（生成器と校正 API）・M6（Web）の成果が、これで初めて利用者に届いた** —— v0.2.0 以来である。**続けて v0.4.0 を 2026-09-12 に公開した** —— M7（`dnn` の opt-in profile ほか）と 2026-09 の API 拡張が、これで初めて届いた。**PNG の ARM 加速を Android / iOS / macOS arm64 で失う退行を含む**（リリースノートに明記してある）。**版番号は当初「v0.3.0 を飛ばして v0.4.0」と書いていたが覆した** —— v0.3.0 という名前で世に出た物が 1 つも無いことを実測したためで、2026-08-31 の下書き（生成物が 1 つも入っていない）と tag は破棄して `03a4557` に打ち直した。**OpenUPM は v0.3.0 のとき、公開の約 4 時間後に `dist-tags.latest` を切り替えた**（pipeline は `state=2` / `commit=03a4557` —— **打ち直した tag に紐づいたまま配信された**）。**v0.4.0 が配信されたかどうかは、確かめてから書くこと** —— レジストリは Release より数時間遅れるので、公開直後にここへ「配信している」と書くと嘘になる。**配布の完了条件 7 件はすべて満たした。****配布の記録は `docs/roadmap.md` の「配布 その 5」にある。** 判定表はいずれも roadmap にある）
...
| M5 | binding specification と generator。**`imgcodecs` は M3.5 で出した**（`ocvu_imencode` / `ocvu_imdecode`、2026-08-30）ので、ここには残っていない。**生成の仕組みは 2026-09-01 に成立し、完了条件 4 件を満たした。残る 1 件は新しい OpenCV module（`geometry` / `calib` / `features` / `objdetect`）の追加**で、別の subsystem（`COMPONENTS` / `Modules` / notice / 成果物の大きさ / 依存 allowlist が同時に動く）なので当初は次の計画に分けた —— 混ぜると「生成が壊れたのか module が壊れたのか」が切り分けられない。**続く 3 つの計画で条件 2 を閉じた** —— `objdetect`（QR の符号化・復号）と `features`（ORB 検出）、`geometry`（射影変換の推定）、カメラの歪み補正（`ocvu_undistort` / `ocvu_find_chessboard_corners`）、そして **`calib` module と `cv::calibrateCamera`**（2026-09-02。**構成ハッシュが変わり 5 platform 分を作り直した**）。**これで 5 件すべてを満たした。** 判定は roadmap の M5 節 |
| **M6** | **Web / Wasm（Unity 同梱 Emscripten と整合）—— 完了条件 5 件すべてを満たした**（2026-09-03）。**Web は 6 つ目の platform で、クロスコンパイルと静的ライブラリの両方**（iOS の 2 つの性質を同時に持つ）。**このマイルストーンの価値は、Web でしか出ない欠陥を 7 件捕まえたことにある** —— うち 1 件は **`CLAUDE.md` の中核の不変条件（例外を ABI の外へ出さない）が Web でだけ黙って成立していなかった**もので、**ブラウザで OpenCV が実際に投げるまで誰も知らなかった**（Emscripten は既定で C++ 例外を無効にする。`throw` は残るのに `catch` が 1 つも組み込まれない）。**Web にだけ在る制限**: `imgcodecs` は JPEG のみ。判定表は roadmap の M6 節 |
| **M7** | **Optional profiles と性能 —— 完了条件 5 件すべてを満たした**（2026-09-10。判定は roadmap の M7a / M7b / CUDA / M7c の 4 節に分かれ、索引は `M7 の判定`）。**`dnn` は opt-in profile として出した**（`OCVU_PROFILE_DNN`。native binary には既定で入っており、切っているのは C# の assembly だけである）。**OpenCV 5 の新しい DNN エンジンはここ** —— 差になりうるのは**新しいエンジンのほう**であって、**「競合に DNN が無い」ではない**（Enox は OpenCV DNN と Unity Sentis を切り替える仕組みを持つ。ただし OpenCV 4.13 系なので 5 の新エンジンではない。[競合調査](docs/unity-opencv-integration-research-and-plan.md) §4.2）。~~Unity には推論の代替があるので前倒ししない。~~ **2026-08-30 の上流調査で「5.0 で DNN を作り込むと 5.1 で作り直しになる」根拠が付き、`dnn` を足す前に native bridge を module 単位に分ける決定を入れた。** その決定は **2026-09-08 に所有者が解除した** —— **上流の実測（旧エンジンの削除・`enum EngineType` の再番号）はいまも成立しており、5.1 が来たら作り直しが要ると見込んでよい。** 変わったのは、所有者がその費用を承知のうえで受け入れると判断したことだけである。決定の内訳・前提条件・一次情報・解除の経緯は roadmap の M7 節にある（**ここに再掲しない** —— 2 箇所が同時に古くなる） |
...
2. 実装計画があればそれに従う。M0 の計画は `docs/superpowers/plans/2026-08-25-m0-tdd-harness.md`、M1 の計画は `docs/superpowers/plans/2026-08-25-m1-opencv-build.md`、M2 の計画は `docs/superpowers/plans/2026-08-26-m2-windows-vertical-slice.md`（いずれも実施済み。M2 は完了条件 8 件すべてを満たした）、M3 の計画は `docs/superpowers/plans/2026-08-28-m3-desktop-three-platforms.md`（Task 8 まで実施済み。完了条件 6 件すべて達成。v0.1.0 を公開し、Linux の欠陥を直した v0.1.1 を出した）、M3.5 の計画は `docs/superpowers/plans/2026-08-30-m3.5-distribution-shape.md`（実施済み。**完了条件 6 件をすべて満たした** —— 判定は `docs/roadmap.md` の M3.5 節）、M4 の計画は `docs/superpowers/plans/2026-08-30-m4-mobile.md`（実施済み。**9 件中 6 件**。件数の正本は roadmap の M4 判定表）、M5 の計画は `docs/superpowers/plans/2026-08-31-m5-binding-generator.md`（実施済み。**5 件中 4 件。条件 2 は計画の側で意図的に外してある**）、その条件 2 の前半を出した計画は `docs/superpowers/plans/2026-09-01-m5-modules-objdetect-features.md`（実施済み。**`objdetect` / `features`、続けて `geometry` を出して条件 2 は「部分的に満たした」になった。`calib` は出していない**）。続けてカメラの歪み補正を出した計画は `docs/superpowers/plans/2026-09-01-m5-calib-undistort.md`（実施済み。`ocvu_undistort` / `ocvu_find_chessboard_corners` を `calib` module を足さずに出した）、**条件 2 を閉じた計画は `docs/superpowers/plans/2026-09-02-m5-calib-camera.md`**（実施済み。**`calib` module を足し `cv::calibrateCamera` を出した。5 件すべてを満たした**）。**M6 の計画は `docs/superpowers/plans/2026-09-03-m6-web-wasm.md`**（実施済み。**Web を 6 つ目の platform として足し、完了条件 5 件すべてを満たした**）。**M7 の計画は 4 本ある** —— `docs/superpowers/plans/2026-09-05-m7-profiles-and-performance.md`（**仕様であって計画ではない。** 3 本が共有する正本）、`-m7a-low-copy-and-benchmarks.md`（実施済み。完了条件 2・3）、`-m7b-module-separation.md`（実施済み。完了条件 4）、`-m7c-dnn-profile.md`（実施済み。完了条件 1。**冒頭の「着手してよいという合図ではない」は 2026-09-08 の解除で失効している**）。**完了条件 5 は計画を経ない文書上の決定として閉じた**（roadmap の `CUDA / cuDNN 同梱の判定`）。**M7 が最後のマイルストーンなので、次の計画は無い。**
...
| リンクしている 9 module の API allowlist | M2 の 9 本に、M3.5 / M5 / 2026-09 の API 拡張 / M7c（`dnn`）の追加を足したもの。**関数名をここに並べない** —— 1 本足すたびにこの行だけが古くなる。**内訳は同文書 §3.5〜§3.14、本数は §3 の冒頭が数える** | 同 §3・§3.5〜§3.14 |
...
| 必須チェック | **29 本。** `ci-native` の 3 job（`Windows x64 (L1 + L3)` / `macOS arm64 (L1 + L3)` / `Linux x64 (L1 + L3)`）、`ci-sanitizers` の 2 job（`Windows x64 AddressSanitizer (L2)` / `Linux x64 ASan+LSan (L2)`）、`ci-lint` の 4 job（`Workflows (actionlint)` / `Shell scripts (shellcheck)` / `PowerShell (PSScriptAnalyzer)` / `Documentation links`）、`codeql` の 2 job（`Analyze c-cpp` / `Analyze csharp`）、`ci-unity` の 2 job（`Unity EditMode (Linux)` / `Unity Standalone (Linux)`）、**`release` の 6 job（`Package windows-x64` / `macos-arm64` / `linux-x64` / `android-arm64` / `ios-arm64` と `Assemble the release assets`）、`ci-native` のモバイル 2 job（`Android arm64-v8a (cross-build)` / `iOS arm64 (cross-build)`）、M6 の Web 3 job（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）**。2 本 → 5 本（M3 完了時）→ 13 本（2026-08-29、PR #30 が 13 本すべてを緑にしたのを見てから拡張）→ **21 本（2026-08-31。`release.yml` を PR でも走らせるようにして 6 本、モバイルのクロスビルド 2 本を足した。どちらも main と PR の両方で安定して緑になったのを見てから拡張した）** → **29 本（2026-09-11。`ci-unity` に足した 5 本 —— `Unity Graphics (Linux)` / `Unity DnnEditMode (Linux)` / `Unity DnnStandalone (Linux)` / `UPM tarball install (Linux)` / `Publish the benchmarks` —— を昇格した。**これまでの拡張と違い、緑の実績を積む前に所有者の判断で昇格させた**：実績は 1 ブランチ上で多くても 3 run である。**昇格にあわせて `benchmarks` job に `if: !cancelled()` を足した** —— 素の `needs:` だと依存が落ちたとき skip になり、**GitHub は skip を合格として通す**）→ **その前は 24 本（2026-09-10。M6 が足した Web の 3 本を昇格した —— #63 以降に merge された 11 本の PR（#63〜#70 と #72〜#74）すべてで 3 本とも success になり、「緑の実績が浅い」という当初の理由が失効したため）**。2026-09-10 に API で実測し、この表と一致 | 全部 pass しないと merge できない |
| 必須でないもの | **13 本。** `ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` 1 本、`codeql` の集約 check `CodeQL` 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、そして **game-ci が `checkName` から作る check run 6 本**（`EditMode results` / `Standalone results` / `Graphics results` / `DnnEditMode results` / `DnnStandalone results` / `Tarball results`）。**後者を必須にしないのは、PR では `neutral` になり GitHub がそれを合格として通すからである** —— 必須にしても止める力は増えない。**測る時刻に注意する** —— game-ci の `* results` は job の終盤に作られるので、**走っている途中に数えると出てこない**（この行は 1 度 12 本と書いて直した。42 件出揃う前の 36 件の時点で数えていた）。**`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、`codeql` の集約 check `CodeQL` の 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`ci-unity` の `EditMode results` / `Standalone results` の 2 本（PR では skip される）。** **M6 が足した Web の 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）は 2026-09-10 に必須へ昇格した** —— モバイルの 2 本と同じ手順である（根拠は上の行に 1 度だけ書いてある）。**したがって Web のレーンは、いまは赤ければ merge を止める。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
...
例外が **2 箇所**ある。`README.md` の "Almost every lane that runs on a pull request
blocks a merge." の段落と、`README.ja.md` の「pull request で走るレーンのほとんどが
merge を止めます」の段落である。どちらも外部の利用者向けに同じことを述べており、
ここを指すわけにいかない（`CLAUDE.md` はエージェント向けの文書である）。
**必須チェックを増減したら、その 2 つも一緒に直すこと。**
リポジトリ内で二重に書かれているのはこの 2 箇所だけ、という状態を保つ。
**`README.ja.md` は `README.md` の翻訳なので、片方だけ直すと 2 つが食い違う。**
...
- ~~**`OCVU_PROFILE_DNN` を立てた Unity。**~~ **2026-09-11 に閉じた。**
  `ci-unity.yml` の `DnnEditMode` / `DnnStandalone` レーンが
  `ProjectSettings.asset` に define を書いてから Unity を走らせる。実測
  （run 34612557397）: EditMode 59 passed で
  `OCVU_PROFILE_DNN is defined and the dnn assemblies are compiled` が
  3 つの assembly 分出力され、**stripping 済みの IL2CPP Player で 40 passed**
  （`AbiSurfaceDnnPlayerTests.EveryDnnEntryPointIsReachable` を含む）。
  **穴だった「Unity と IL2CPP の側」が、これで両方とも埋まった。**
  それ以前から CI が見ていたもの（L1 / L3 / shim / 非既定 profile の
  `dotnet build`）は変わらず走る
...
- ~~**全部入りの package。**~~ **2026-09-11 に閉じた。** `ci-unity.yml` の
  `tarball` job が、6 platform を重ねて固めた tarball を使い捨ての Unity
  プロジェクトへ導入し、EditMode を走らせる。プロジェクトの作り方は
  `dev.ps1 test-unity-tarball -PrepareOnly` に寄せてあり（**CI に書き写さない**
  —— 分けると「導入を確かめた tarball」と「配る tarball」が別物になる）、
  Unity の起動だけが game-ci である。UPM が **`.tgz` で解決したこと**まで
  CI が見る。**閉じる前の実測はローカル（Windows）だけだった**（**6 platform の全部入りでは 2 回走らせてある** —— 2026-09-03 に `release.yml` の空撃ちが作った tarball を材料に、2026-09-04 に公開する下書きの asset そのものを材料に。どちらも `native plugins present: 6` / exit 0（**件数を写さない** —— 正本は上の `test-unity-tarball` の行で、そちらは 2026-09-10 に取り直してある。当時の値をここに残すと、同じファイルの中に 2 つの件数が並ぶ）。詳細は `docs/roadmap.md` の「配布 その 5」）
- **mobile 実機の挙動。** Android / iOS は CI がビルドし、全部入りにも入るが、
  **実機で動かした実績が 1 度も無い**（M4 の完了条件 3 件が閉じていない。
  実機が要る 2 件は `docs/m4-device-verification.md` の手順書に落としてある）
- **Web の browser E2E が動かすブラウザは 1 つだけ。** 実物の Player を
  実際に起動して P/Invoke まで通すが、**Linux の headless Chromium である**
  —— 他のブラウザ・実機・モバイルのブラウザの挙動は見ていない
- **`dev.ps1 test-unity-web` は、どの workflow からも走らない。**
  CI の `web-e2e` job は Player を自分で建て、`assert-emscripten-version.ps1` と
  `run-web-e2e.ps1` を直接呼ぶ。**2026-09-11 に、これが唯一の「直接呼ぶ」形に
  なった** —— それまで並んでいた `test-unity-tarball` は、CI が
  `dev.ps1 test-unity-tarball -PrepareOnly` を**実際に通る**形へ寄せたためである
  （プロジェクトの作り方が 1 つになり、写しが消えた）。web も同じ形にできるかは
  見ていない
- **`RenderTexture` を使う公開 API の GPU 経路（M7a）のうち、Player 側。**
  **Editor 側は 2026-09-11 に閉じた** —— `ci-unity.yml` の `Graphics` レーンが
  `-testCategory Graphics` で走り、`ToMat` / `RequestMat` が実際に画素を運ぶ
  経路を CI が実行する（実測 run 34612557397: 7 passed。
  `AGraphicsDeviceIsPresent` が通ったので**コンテナには実物の
  graphics device が在り**、`AsyncMatchesSync` も通ったので
  `AsyncGPUReadback` も動く）。**成立の根拠は、game-ci の Editor が
  `-nographics` で走っていないことである** —— コンテナの `unity-editor` は
  `xvfb-run -ae /dev/stdout "$UNITY_PATH/Editor/Unity" -batchmode "$@"` で、
  仮想 X の下から起動する。**「CI では原理的に無理」と書いてあったが、
  誰も測っていなかった。**
  **残っているのは Player（IL2CPP）側である** —— game-ci の `run_tests.sh` は
  Standalone Player を `xvfb-run ... -batchmode -nographics` で起動しており、
  **`-nographics` は action 側が固定していてこちらからは外せない。**
  したがって GPU に依る経路を Player で確かめる術は、いまの構成には無い
...
| skill | いつ使うか |
| --- | --- |
| `add-abi-function` | `ocvu_` の ABI 関数を追加・変更・削除するとき。L1 → **spec に 1 エントリ + `dev.ps1 generate`** → 実装 → L3 → status 同期の TDD 順序と、所有権・バッファ・例外バリアの規約。**M5 以降、宣言は手で書かない** —— ヘッダにも `NativeMethods.cs` にも手で足さない（足せば `verify-generated` が落とす）。**新しい OpenCV module を足すなら、先にリンクを実証する節**も読むこと —— `COMPONENTS` に足すだけでは binary は 1 バイトも増えず、`cv::` を実際に参照する L1 テストだけが証拠になる。**2026-09 の API 拡張で 1 つ加わった** —— **`docs/api-reference.md` に名前を書くのも同じ commit である**（速いレーンが「spec の全関数名が api-reference に現れること」を fail-fast で見るので、**「文書は最後にまとめる」は成立しない**）。**M7 で 3 つ加わった** —— **`OCVU_SOURCES` はもう手で書く一覧ではない**（正本は `native/modules.cmake` の `OCVU_MODULE_<module>`。新しい module では `#include` 1 行に加えてここに 2 行要る）／ **profile を持つ module に足すときの分岐**（出力先・クラス名・assembly が変わり、**到達性テストの呼び出し元を自分で作らないと生成物が stripping の対象になる**。**2026-09-11 から `ci-unity.yml` の `DnnEditMode` / `DnnStandalone` レーンが `OCVU_PROFILE_DNN` を立てて走るので、確認は手動ではない** —— ただし profile を新設したら、そのレーンを足すのは自分の仕事である）／ **逆向きの検査は 2 本**（ソースを読む `BindingGenerator.Tests.ps1` と、binary の export 面を読む `verify-exported-symbols.ps1`）|
| `add-a-platform` | **対象 platform を足す・外す・変えるとき。** 一覧を持つ場所は**多い**（**数を写さない。正本は skill 側の表の行数である** —— skill からも数を落とした。**M7 で 2 つ増えたのに「17」だけが取り残された**ため。M6 で数え直したときは**リポジトリの外にもあった**）、語彙が違うので grep 1 回では揃わない。`.meta` のキー名 / ファイル名の platform 間衝突 / クロスでの `find_package` の閉じ込め / 静的ライブラリの依存の束ね / 新しい third-party など、**クロスビルドが緑になってから CI で 8 回落ちた**罠を、踏んだ順に並べてある。**M7 で 3 つ加わった** —— `release.yml` の「未知の platform は落とす」門は **2 つではなく 3 つ**（M7b が公開面の検査を足した）／表に 2 行（`verify-exported-symbols.ps1` の配線 2 箇所、全部入り tarball の上限 —— **M7c は実際にここで落ちた**）／**構成ハッシュは `opencv-config.psd1` の内容しか見ないので、toolchain file に書いた flag は復元済み artifact に反映されない**（罠 8 の対になる半分）。あわせて **allowlist の限界**（別のライブラリへ静的に取り込まれた third-party は見えない）も罠 5 に入れた |
| `milestone-complete` | マイルストーンの完了を判定するとき。roadmap の完了条件との実測照合、**文書の陳腐化確認**、CI での確定。**M4 で 2 つ加わった** —— 実機が要る条件は「満たすが未実証」ですらなく人が実行する手順書に落とす / **必須チェックの充足は成功件数ではなく名前で突き合わせる**（GitHub は `skipped` と `neutral` も pass として通す）。**M6 でさらに 3 つ** —— **条件つきで先送りした手順書は、条件が満たされた瞬間にその否定文が嘘になる**（しかも中の「済ませた」印は語句の置換では直らず、**そのまま実行すると中身と説明が食い違った Release が出る**）／ **数が動かなくてもその補集合が動く**（M6 は**必須チェックの本数を 1 本も変えずに、必須でないものの本数を変えた**。数が動いていないので**どの機械も捕まえない**）／ **直すときは強いほうから**（要約や図は目に付くので先に直り、**実行手順と散文が残る**）。**2026-09 の API 拡張でもう 1 つ** —— **`.md` だけを洗うと足りない**（事実は検査スクリプトと実装のコメントにも書かれており、そちらは「文書」と呼ばれないので見落とす。**実際に 4 件踏んだ。どれも CI は緑のまま通る**）。**M7 でさらに 3 つ** —— CI に配線が無いレーンは**判定のときだけが唯一の実行機会**である（**M7 当時それに当たったのは `test-unity-graphics` / `benchmark` で、2026-09-11 に両方とも CI へ配線した。規則そのものは生きている**）／`test-unity-player` は後始末でハングする既知の欠陥を持つ（結果 XML には出るので、プロセスを止めて `assert-unity-results.ps1` を直接掛ける）／**判定を複数の節に割ったら、残数を書かず、名指し参照を自分で洗う**（`ci-lint` のリンク検査は backtick 見出し参照を見ない。**索引になる節を 1 つ置かないと「完了した」と書いた文がどこにも無い状態になる** —— M7 で実際にそうなった）|
| `prove-a-check-works` | テスト・assertion・検証スクリプト・allowlist・CI ゲート・hook を足すか変えるとき。**壊して落ちることを見るまで、その検査は動くと言えない。** M1 の全タスクが同じ欠陥（著者が列挙した形だけを見る）を生んだので手順にした。**M4 で 6 つの節が加わった** —— 壊す前にコミットする / 置換が空振りしていないか確かめる / 手前に別の門があると番人に到達しない / 述語のゆるさ（構造的に常に真・部分一致・大文字小文字・散文に当たる）/ **自分でパースする検査は本物の解釈を代理できない** / **正本を写さず正本から読む**。**M5 でさらに 4 つ** —— **Unity 経由のクラッシュは赤いテストにならない**（無音で 10 分以上返り、結果が 1 バイトも出ない）/ 手前の門の 2 例目（`System.Text.Json` が先に落とすので、足した検査は 1 度も動いていなかった）/ **アンカーの意味**（.NET の `$` は末尾の `\n` の直前にも当たる。「一致したか」ではなく**値全体を覆ったか**を見る）/ **列挙に基づく門を出口側の構造に基づく門へ変える**（ただし下流に検査が無い所にだけ足す）。**M6 でもう 1 つ** —— **負の対照の「形」が想定と違うことがある**（`-msimd128` を外すと検査が赤くなるのではなく**コンパイルが通らない**。より強い形だが**同じ形ではない**ので、同じ言葉で記録してはいけない ——「壊したら落ちた」と書くと次の人は「検査が落ちた」と読むが、**検査は 1 度も走っていない**）。**2026-09 の API 拡張でその対になる形が出た** —— **負の対照がそもそも取れないことがある**（ステレオの入力を縞から模様に変えたとき、BM 側は縞に戻すと落ちたが **SGBM は縞でも視差を見つけるので落ちなかった**）。**M6 のものが「より強い形」なら、こちらは「弱い形」である** ——「壊したら落ちた」と書けないときは、書けないと書く。**M7 で 5 つ** —— **数を見る検査は、誤った読みに満たされる**（同じ形を 3 回作った実例。フォルダを「2 つ以上読めたか」で守ると 4 つのうち 2 つしか読めない誤読が通り、**合計 3 本に増やしても釣り合わせられた**。閉じたのは「合計は十分に大きいか」を「**フォルダを出すはずだった要素は全部出したか**」に変えたときで、**その構造の門 1 本が数の門 2 本を消した**）／**門を何本置くかは本数ではなく、1 本ずつ単独で何を捕まえるかで決める**（対になって初めて 1 つの門になる形と、束にしても釣り合う形の見分け方）／**検査が当たっていた対象が本番の物ではなく代用物だった** 2 例（開発用 binary / 手書き probe）／**負の対照の 3 例目。壊しても落ちず、かつ必要性を確定させる経路がリポジトリに存在しない**（`.clone()`。「検査が働くことを示せていない」と「そのコードが要ることを示せていない」は別の主張である）／**走らないレーンは直した後も同じ速さで腐る**（`test-unity-tarball` が M7a で再発した）|

**hook**（`.claude/settings.json`）

| hook | 契機 | 動作 |
| --- | --- | --- |
| `block-bulk-git-add.sh` | PreToolUse (Bash/PowerShell) | `git add -A` / `git add .` を**拒否**。連結コマンド内も見る。**`git add -u` と `git commit -a` は通す** —— 生成物が履歴に入るのを止めるのは `check-staged-generated-file.sh` の担当で、分担は意図的である（冒頭のコメントが「生成物、一時的なデバッグ変更を巻き込む」と書きながらその 2 つを拾う `-u` を見ていなかったずれは、2026-09-10 にコメント側を直した）|
| `check-unityengine-leak.sh` | PostToolUse (Write/Edit) | **shim がコンパイルする Runtime のフォルダ**への `UnityEngine` 混入を指摘。**一覧を写さず正本から読む** —— 正本は `tests/Managed/CvUnity.Runtime.Shim/CvUnity.Runtime.Shim.csproj` の `<Compile>` 要素のうち **Include が `$(OcvuPackageRuntime)` を根に持つもの**である（リテラルの相対パスで書かれたフォルダは対象外。hook の冒頭が何を除いているかを書いている）。**判定は合計ではなく要素ごとである** —— 合計 3 本の突き合わせは**釣り合わせられる**と実測で分かった（3 パスを持つ要素と 1 つも読めない要素を並べると要素 2 / パス 3 / フォルダ 3 で成立し、1 フォルダが黙って無防備になる）。**写していた頃は実際に古くなった**: M7c が profile 分離で `Runtime/Interop.Dnn` と `Runtime/Dnn` を足し、shim は 4 フォルダをコンパイルするようになったのに hook は 2 のままで、**後の 2 つは素通りしていた**（2026-09-10 に 4 フォルダへ probe を当てて実測し、正本から読む形に直した）|
...
| `check-staged-generated-file.sh` | PreToolUse (Bash/PowerShell) | **手で書き換えた生成物が commit されるのを止める**（2026-09-03 に足した）。`prove-a-check-works` の手順で生成物を壊し、**戻す前にコミットして必須 21 本のうち 3 本を赤くした**のを受けている。**既存の門はどれも止めなかった** —— `block-bulk-git-add.sh` は `-A` と `.` を止めるが **`-u` は素通し**し（コメントは「生成物、一時的なデバッグ変更を巻き込む」と書いているのに実装がずれていた）、`check-generated-file-edit.sh` は **hook の外で書き換えたもの**（`sed` の注入）を見ない。**判定は「生成物が入っているのに `bindings/` が入っていない」である** —— 生成物は `bindings/spec/**` と `bindings/generator/**` からしか変わらないので、元が無ければ手作業である。**痕跡の文字列を列挙しない**（最初はしていた。レビューが両向きに穴だと実測した —— **未知の文字列を通し**、`REMOVED` のような語は **生成物の中身に自然に現れて正当な commit を止める**）。**規則が働くことは履歴で測ってある**: 生成物を触った 25 commit のうち `bindings/` を伴わないのは 2 件で、それは**事故そのものとその修復**である。**`-u` だけを塞ぐ手は採らなかった** —— `git commit -a` が同じ形で残り、**片方だけ塞ぐと塞がれていない動詞に移るだけ**である。**その `-a` を最初は実装が取りこぼしていた**（index を見ていたが、`-a` が stage するのは hook の後）—— **自分が `block-bulk-git-add.sh` について書いた「理由と実装のずれ」を、隣のファイルで再生産していた。** **対象を一覧で持たない**（生成物が先頭 5 行で名乗る規約を見る）ので、生成物が増えても変わらない |
| `block-local-opencv-build.sh` | PreToolUse (Bash/PowerShell) | **OpenCV をローカルでビルドさせない**（2026-09-03 に足した）。`CLAUDE.md` はこれを 3 箇所で書いているが、**3 箇所書いてあっても踏んだ** —— M6 で新しい platform の CMake flag を探すために **5 回続けてローカルで回した**（1 回 10〜40 分、5 回とも失敗）。**これは「検証」ではなく「反復」で、まさに禁じられている使い方である。** **文書は読まれても効かなかったので機械に見させる。** 止めるのは `opencv.ps1 build` だけで、`restore` / `verify` / `status` / `clean` と `dev.ps1 build`（native の側）は通す。**新しい platform でこそ破りたくなる** —— 「CI に投げて 20〜40 分待つ」より速く見えるからだが、**手元の環境は CI と違うので、手元で潰した時間の一部は CI に何も貢献しない**（実測: 5 件中 2 件は手元にしか無い問題だった）。逃げ道は `OCVU_ALLOW_LOCAL_OPENCV_BUILD=1` で、**意図して回したことが履歴に残る形**にしてある。詳細と往復の内訳は `add-a-platform` skill の「OpenCV は CI にビルドさせる」 |
