...
**続けてカメラの歪み補正も出した**（2026-09-02、ブランチ `feat/m5-calib`）。`ocvu_undistort`（`imgproc`）と `ocvu_find_chessboard_corners`（`objdetect`）の 2 関数を足し、C ABI は 26 本、allowlist は 17 本、C# の P/Invoke 宣言は 28 本になった。**`calib` module はまだ足していない** —— 2 本とも既存にリンク済みの `imgproc` / `objdetect` に在る（実測。`native/tests/test_module_linkage.cpp` が固定している）。**それでも条件 2 は「満たした」にはならない** —— カメラ校正は (1) 格子点を見つける、(2) そこから係数を解く、(3) 係数で歪みを補正する、の 3 段からなり、今回出したのは (1) と (3) だけである。**(2) の `cv::calibrateCamera` こそが `calib` を要求する段で、まだ出していない**（足すと構成ハッシュが変わり 5 platform 分の再ビルドを要する。実測 `4785d98e9aad` → `09fcbe260d87`）。利用者は係数をどこか別の手段で得なければ、この 2 本を使えない —— **校正の輪は閉じていない。** 判定は引き続き「部分的に満たした」である（詳細は `docs/roadmap.md` の M5 節）。
...
| 場所 | 内容 |
| --- | --- |
| `bindings/spec/*.json` | **境界の正本（M5）。** module ごとに 1 ファイル（`infra` / `core` / `imgproc` / `imgcodecs` / `objdetect` / `features`）。`schema.json` が形を縛る。**entry 数を数えるのは `docs/api-map.md` の冒頭だけ**である。**ここに 1 エントリ書いて `dev.ps1 generate` を実行するのが、関数を足す唯一の経路である** |
...
| `native/include/ocvu/{infra,core,imgproc,imgcodecs,objdetect,features}.h` | **生成物（M5）。** module ごとの `extern "C"` 宣言。**手で編集しない** —— `dev.ps1 verify-generated` が落とす |
| `docs/api-map.md` | **生成物（M5）。** spec の entry を 1 行ずつ並べた API 対応表。**本数を数えるのはこの表の冒頭だけ**で、他所に数字を写さない。「OpenCV 全対応」のような曖昧な表現を置かないための出口でもある |
| `native/src/` | C ABI 実装（version / last-error / status 表 / debug throw・crash / OpenCV version・build information / `Mat` のライフサイクルと buffer 転送 / imgproc 3 関数 / **imgcodecs の encode・decode 2 関数** / objdetect の QR 符号化・復号 2 関数 / features の ORB 検出 1 関数 / geometry の射影変換推定 1 関数 / **カメラの歪み補正 2 関数（`native/src/ocvu_calibration.cpp`。`imgproc` の `ocvu_undistort` と `objdetect` の `ocvu_find_chessboard_corners` が 1 ファイルに同居する ── 用途は 1 つだが 2 module にまたがるため）**） |
| `native/tests/` | L1 の GoogleTest と、意図的にクラッシュ・ハングする `ocvu_probe` |
| `cmake/run_expect_failure.cmake` | 「失敗するはずのコマンド」を走らせる CTest ドライバ |
| `cmake/FindOpenCvUnityDeps.cmake` | `third_party/opencv/<hash>/` を探して OpenCV を取り込む。**`COMPONENTS` の行がこのプラグインの実際のリンク対象を決める**（現在は `core imgproc imgcodecs objdetect features geometry`。**ただし `flann` と `geometry` は `features` / `objdetect` の依存として推移的に引かれるので、書かなくてもリンク行に入る** —— 明示は意図の宣言であって、no-op であることは実測で確かめてある）—— `tools/opencv-config.psd1` の `Modules` は OpenCV 側が何をビルドするかであって、こちらが何をリンクするかではない |
...
## マイルストーン（現在地: **M5（binding specification と generator）を実装し、完了条件 5 件のうち 4 件を満たした**（計画は `docs/superpowers/plans/2026-08-31-m5-binding-generator.md`）。**条件 2（`geometry` / `calib` / `features` / `objdetect` の追加）は当初計画の側で意図的に外してあり、次の計画に送った。その次の計画（`.superpowers/sdd/2026-09-01-m5-modules-objdetect-features/`）で `objdetect` / `features` を出し、条件 2 は「部分的に満たした」まで進んだ** —— `geometry` は 2026-09-01 に出した（`ocvu_find_homography`。**リンクは無料だった** —— `features` / `objdetect` の依存として推移的に引かれており、`COMPONENTS` に足す必要すら無かった）。**続けてカメラの歪み補正（`ocvu_undistort` / `ocvu_find_chessboard_corners`）も出した**（2026-09-02、`imgproc` と `objdetect` の関数で `calib` は使っていない。C ABI は 26 本、allowlist は 17 本）が、**係数を求める `cv::calibrateCamera` はまだ出していない**ので条件 2 は依然「部分的に満たした」である。**`calib` module 自体はまだ足していない** —— `Modules` に無いため構成ハッシュが変わり 5 platform 分の再ビルドを要する（実測 `4785d98e9aad` → `09fcbe260d87`）。**M4（Mobile）も完了していない** —— 完了条件 9 件のうち 4 件が閉じていない（iOS 実機 / lifecycle / macOS 上の Unity / Windows IL2CPP の結論。うち後ろ 2 件は 2026-08-31 に「CI では閉じない」と結論した）。**v0.2.0 が最新の公開版で、v0.3.0 は下書きのまま止めてある**（2026-09-01 に公開しないと決めた）。**その下書きをそのまま公開してはならない** —— 作った時点は M5 が main に入る前で、生成物が 1 つも入っていない。判定表はいずれも roadmap にある）
...
| M5 | binding specification と generator。**`imgcodecs` は M3.5 で出した**（`ocvu_imencode` / `ocvu_imdecode`、2026-08-30）ので、ここには残っていない。**生成の仕組みは 2026-09-01 に成立し、完了条件 4 件を満たした。残る 1 件は新しい OpenCV module（`geometry` / `calib` / `features` / `objdetect`）の追加**で、別の subsystem（`COMPONENTS` / `Modules` / notice / 成果物の大きさ / 依存 allowlist が同時に動く）なので当初は次の計画に分けた —— 混ぜると「生成が壊れたのか module が壊れたのか」が切り分けられない。**続く計画で `objdetect`（QR の符号化・復号）と `features`（ORB 検出）、さらに `geometry`（射影変換の推定）を出し、条件 2 は「部分的に満たした」まで進んだ。続けてカメラの歪み補正（`imgproc` の `ocvu_undistort` と `objdetect` の `ocvu_find_chessboard_corners`）も出したが、`calib` module 自体と係数を求める `cv::calibrateCamera` は依然として出していない**ので判定は変わらない |
...
2. 実装計画があればそれに従う。M0 の計画は `docs/superpowers/plans/2026-08-25-m0-tdd-harness.md`、M1 の計画は `docs/superpowers/plans/2026-08-25-m1-opencv-build.md`、M2 の計画は `docs/superpowers/plans/2026-08-26-m2-windows-vertical-slice.md`（いずれも実施済み。M2 は完了条件 8 件すべてを満たした）、M3 の計画は `docs/superpowers/plans/2026-08-28-m3-desktop-three-platforms.md`（Task 8 まで実施済み。完了条件 6 件すべて達成。v0.1.0 を公開し、Linux の欠陥を直した v0.1.1 を出した）、M3.5 の計画は `docs/superpowers/plans/2026-08-30-m3.5-distribution-shape.md`（実施済み。**完了条件 6 件をすべて満たした** —— 判定は `docs/roadmap.md` の M3.5 節）、M4 の計画は `docs/superpowers/plans/2026-08-30-m4-mobile.md`（実施済み。**9 件中 5 件**）、M5 の計画は `docs/superpowers/plans/2026-08-31-m5-binding-generator.md`（実施済み。**5 件中 4 件。条件 2 は計画の側で意図的に外してある**）、その条件 2 の前半を出した計画は `docs/superpowers/plans/2026-09-01-m5-modules-objdetect-features.md`（実施済み。**`objdetect` / `features`、続けて `geometry` を出して条件 2 は「部分的に満たした」になった。`calib` は出していない**）。続けてカメラの歪み補正を出した計画は `docs/superpowers/plans/2026-09-01-m5-calib-undistort.md`（実施済み。**`ocvu_undistort` / `ocvu_find_chessboard_corners` を `calib` module を足さずに出した。`cv::calibrateCamera` は出していないので条件 2 は引き続き「部分的に満たした」である**）。M6 以降の計画はまだ無いので、`superpowers:writing-plans` で先に書く
...
| `core` / `imgproc` / `imgcodecs` / `objdetect` / `features` / `geometry` API の allowlist | Mat の create / release / clone / get_info / copy_from_buffer / copy_to_buffer と cvtColor / resize / GaussianBlur（M2 の 9 本）に、M3.5 の imencode / imdecode、M5 の qr_encode / qr_decode / orb_detect / find_homography / undistort / find_chessboard_corners を足した 17 本 | 同 §3・§3.6〜§3.8 |
