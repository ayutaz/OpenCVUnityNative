...
| 必須でないもの | **`ci-unity` の `Plugin windows-x64` / `Plugin macos-arm64` の 2 本**（2026-08-30 に追加）。**必須にしないと決めた** —— この 2 つが落ちると `unity` job は `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める。**skip は required check を通す**（GitHub の文書）ため、`needs:` を足すだけでは止まらない。そこは workflow の構造で塞いである。`build-opencv` / `nightly` / `release` は PR では起動しない（構成変更・定期実行・tag が trigger）ので、そもそも必須にできない | — |
