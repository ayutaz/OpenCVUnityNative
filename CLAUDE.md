...
| `./tools/dev.ps1 test-unity-editmode` | L4（Unity EditMode、Mono、**58 件**。`-testCategory '!Graphics'` で Graphics レーン側のテストを除外する） | 約 27 秒（増分。2026-08-28 実測。**Unity 6000.0.82f1・EditMode 10 件のときの値。** その後 M3.5 で 16 件、M4 で 33 件、M5 で 34 件、2026-09 の API 拡張で 48 件、M7a で 53 件、M7b で `ProfileGatingTests`（4 件）が加わり 57 件になったが所要時間は取り直していない。**2026-09-11 に 58 件になった** —— `CiVisibilityTests` を消したためである（Graphics レーンが CI に配線されたので、「CI から見えないテストの一覧」という契約自体が用済みになった。同 class の docstring が「配線できたら検査ごと消してよい」と書いていた）。**その直前の 59 件は 2026-09-10 に実測した値**（`unity-editmode.xml` の `testcasecount="59"`）。旧「58 件」は M7 squash（`be5615f`）の時点では正しかった——`tests/UnityProject/Assets/Tests/EditMode/*.cs` の `[Test]`/`[UnityTest]` 属性行を数えると `be5615f` で 65 件、うち Graphics カテゴリ 7 件を除いて 58 件。差の 1 件は M7c（`e1b0930`、"dnn を opt-in profile として足す"）が `ProfileGatingTests.cs` に足した `TheDnnPublicApiAssemblyIsAlsoAbsentWithoutItsDefine`（同ファイルの `[Test]` 数が 5→6）で、同じ数え方で `e1b0930` は 66 件、Graphics 7 件を除いて 59 件になる——この行を更新する commit がその後に続かなかっただけである） |
...
| 必須でないもの | **9 本**（PR #74 の 33 checks から必須 24 本を引いて実測）。**`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、`codeql` の集約 check `CodeQL` の 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`ci-unity` の `EditMode results` / `Standalone results` の 2 本（PR では skip される）。** **M6 が足した Web の 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）は 2026-09-10 に必須へ昇格した** —— モバイルの 2 本と同じ手順である（根拠は上の行に 1 度だけ書いてある）。**したがって Web のレーンは、いまは赤ければ merge を止める。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
...
- **`dev.ps1 test-unity-web` は、どの workflow からも走らない。**
  ...
  `run-web-e2e.ps1` を直接呼ぶ。**上の `test-unity-tarball` と同じ形である**
