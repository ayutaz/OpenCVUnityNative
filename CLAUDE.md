...
| M7 | Optional profiles と性能。**OpenCV 5 の新しい DNN エンジンはここ** —— 差になりうるのは**新しいエンジンのほう**であって、**「競合に DNN が無い」ではない**（Enox は OpenCV DNN と Unity Sentis を切り替える仕組みを持つ。ただし OpenCV 4.13 系なので 5 の新エンジンではない。[競合調査](docs/unity-opencv-integration-research-and-plan.md) §4.2）。Unity には推論の代替があるので前倒ししない。**2026-08-30 の上流調査で「5.0 で DNN を作り込むと 5.1 で作り直しになる」根拠が付き、`dnn` を足す前に native bridge を module 単位に分ける決定を入れた。** 決定の内訳・前提条件・一次情報は roadmap の M7 節にある（**ここに再掲しない** —— 2 箇所が同時に古くなる） |
...
| 必須でないもの | **`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、そして M6 が足した 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）。** **M6 の 3 本を必須にしないのは、緑の実績がまだ浅いからである** —— **2026-09-03 に PR #63 で 3 本とも初めて緑になり、merge 後の main でも `Web wasm32 (cross-build)` と `Web browser E2E` が緑になった**（`Package web-wasm` は `release.yml` が main の push で走らないので PR の 1 回だけである）。それでも `CLAUDE.md` の規律は「**安定して緑になったのを見てから必須へ加える**」で、**PR 1 回 + main 1 回は「安定して」ではない。** **モバイルの 2 本も M4 の直後は必須外で、2026-08-31 に昇格した。** 同じ手順を踏む（**PR と main で安定して緑になったら昇格する**）。**帰結を直視すること: いま Web のレーンは赤くても merge を止めない。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
...
- **全部入りの package。** 3 platform を 1 つに束ねる形自体は M3.5 で成立したが、
  **それを確かめるレーン（`dev.ps1 test-unity-tarball`）はどの workflow からも
  走っていない。** このレーンの実測はローカル（Windows）だけである（**6 platform の全部入りでは 2 回走らせてある** —— 2026-09-03 に `release.yml` の空撃ちが作った tarball を材料に、2026-09-04 に公開する下書きの asset そのものを材料に。どちらも `native plugins present: 6` / `34 passed` / exit 0。詳細は `docs/roadmap.md` の「配布 その 5」）
