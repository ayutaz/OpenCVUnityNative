...
| `./tools/dev.ps1 test` | **既定**。tools の速いテスト + `verify-generated` + L1 + L3 | **約 2 分 24 秒**（増分、成果物が最新。2026-09-06 実測、exit 0。同じ条件で 2 回測って同じ値）。**M3.5 以前の「約 21 秒」→ 2026-09-01 の「約 65 秒」から、さらに伸びた** —— tools の速いテストが 7 本になり、うち `BindingGenerator.Tests.ps1` は**合成した非既定 profile の生成物を `dotnet build` に掛ける**（単体 36 秒）、`Benchmarks.Tests.ps1` は `pwsh` を 6 回起こす（単体 10 秒） |
| `./tools/dev.ps1 test-tools` | `tools/tests/` の速いレーン（OpenCV 構成・ハッシュ無効化・**生成物と spec の一致**・**Unity と Emscripten の対応表**・**tarball の大きさ**・**binary の公開面が spec と一致すること（M7b）**・**benchmark の結果が publish できる形かどうか**）。**本数を写さない。正本は `tools/dev.ps1` の `$ToolsTestScriptsFast` である**（M7 で 2 本増えた。この列挙もそれに合わせて足した——列挙自体を数の代わりにしない） | 約 18 秒（**2 本だった頃の値**） |
...
「ローカルループは秒単位を死守する」という不変条件（本ファイル下部）と、`test` の実測（2026-09-06 で約 2 分 24 秒）はすでに緊張関係にある。M1 ではこれを**受け入れて記録するに留めており、解消していない**。**M6 でさらに 4 本目の `pwsh -NoProfile` 起動が加わったので、65 秒はもう上限ではない**（M3.5 のときと同じく、**前提が変わったことは記録し、再計測はしていない**）。**M5 でこの緊張は 1 段強まった** —— M1 の 65 秒は**ソースを変えた**ときの値だったが、2026-09-01 の 65 秒は**何も変えていない**ときの値である（tools のテストが 3 本になり、`verify-generated` と L3 の 2 つ目の assembly で `dotnet` の起動が 2 回増えた）。**M7b でさらに 1 段強まり、実測は 2 分 24 秒になった**（tools の速いテストが 7 本になり、そのうち 1 本は `dotnet build` を回す）—— **今度は再計測してある**。着手するなら configure の結果を跨いで再利用するか、OpenCV に依存しないレーンを分けることになる。
