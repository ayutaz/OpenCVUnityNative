...
**M2（Windows vertical slice）は、8 件の完了条件のうち 7 件を満たしている。残る 1 件は未達で、M2 を完了と称さない。** `Mat` のライフサイクル（create / release / clone / get_info / copy_from_buffer / copy_to_buffer）と `imgproc` 3 関数（cvtColor / resize / GaussianBlur）が C ABI にあり、所有権契約（二重解放・解放後アクセス・buffer の長さ/stride/NULL 検証）が L3 でテストされ、Unity Editor (Mono) と Windows IL2CPP Player の両方で同じ smoke test が通る。未達の 1 件は「`ci-unity.yml` が CI 上で L4/L5 を実行すること」— workflow ファイル自体は存在するが、Unity ライセンスの GitHub Secrets が未登録のため一度も実行されていない（`gh run list --workflow=ci-unity.yml` は default branch に無く 404、`gh secret list` に `UNITY_*` は 0 件）。資格情報の登録はユーザーの操作であり、エージェントが代行するものではない。判定の詳細は `docs/superpowers/plans/2026-08-26-m2-windows-vertical-slice.md` の実装計画と、その進行記録（`.superpowers/sdd/2026-08-26-m2-windows-vertical-slice/progress.md`）にある。
...
## マイルストーン（現在地: M2 は 8 件中 7 件達成、残り 1 件はユーザーの Secrets 登録待ち）
...
| **M2** | **Windows vertical slice。API の広さではなく ownership / stride / エラー / IL2CPP の正しさを確定 — 8 件中 7 件達成。残る条件（`ci-unity.yml` が CI 上で L4/L5 を実行すること）は Unity ライセンスの GitHub Secrets が未登録のため未達。登録はユーザーの操作で、エージェントは代行できない** |
