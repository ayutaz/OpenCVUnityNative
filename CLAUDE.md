...
**調査・企画段階であり、実装コードもビルドシステムもまだ存在しない。** リポジトリには `docs/` のみがあり、git commit も 0 件（`main` は空）。

したがって build / lint / test コマンドは存在しない。存在しないコマンドを提案・実行しないこと。実装を開始する場合は、下記「初回実装時の起点」に従う。
...
- **例外を ABI の外へ伝播させない**。OpenCV / C++ 例外は status code + thread-local last-error に変換する。FFI 境界を越える unwind は未定義動作になり得る。すべての公開 ABI 関数は `OCVU_TRY_BEGIN` / `OCVU_TRY_END` で本体を囲む。
...
| M | 目的 |
| --- | --- |
| **M0** | **自動 TDD ハーネスの成立（OpenCV 非依存）。** 反復速度の土台を他の何よりも先に固定する |
| M1 | OpenCV 5.0.0 の再現可能ビルド。CI がビルドし artifact 配布、ローカルは download のみ |
...
2. 実装計画があればそれに従う（M0: `docs/superpowers/plans/2026-08-25-m0-tdd-harness.md`）。無ければ `superpowers:writing-plans` で先に書く
...
ディレクトリ構成の想定は計画書 §10 にある（`native/`、`bindings/spec|generator|generated-checks/`、`Packages/com.ayutaz.opencv-unity-native/`、`tests/UnityProject/`、`tools/`、`cmake/`、`.github/workflows/`）。M0 の実装計画にファイル単位の割り当てがある。
