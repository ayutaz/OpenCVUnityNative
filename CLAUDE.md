...
| 必須でないもの | **12 本**（2026-09-11 に PR #76 で実測。`gh pr checks 76` の 36 checks から必須 24 本を引き、**集合演算で名前を突き合わせた**）。内訳は `ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` 1 本、`codeql` の集約 check `CodeQL` 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、そして **2026-09-11 に足した 5 本**（`Unity Graphics (Linux)` / `Unity DnnEditMode (Linux)` / `Unity DnnStandalone (Linux)` / `UPM tarball install (Linux)` / `Publish the benchmarks`。**必須にするかは決めてある —— しない**。理由と昇格の条件は `docs/roadmap.md` の「GPU 経路を CI に載せる」）。**数え方に注意が要る** —— game-ci は `checkName` から `EditMode results` などの check run を commit 上に作るが、**`gh pr checks` はそれを列挙しない**（同じ SHA の check-runs は API では 55 件ある）。以前ここに書いてあった「9 本」はその 2 本を数に入れていたので、**同じ方法で数え直すと 7 本だった**。**`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、`codeql` の集約 check `CodeQL` の 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`ci-unity` の `EditMode results` / `Standalone results` の 2 本（PR では skip される）。** **M6 が足した Web の 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）は 2026-09-10 に必須へ昇格した** —— モバイルの 2 本と同じ手順である（根拠は上の行に 1 度だけ書いてある）。**したがって Web のレーンは、いまは赤ければ merge を止める。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
