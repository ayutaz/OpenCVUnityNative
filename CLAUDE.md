...
| 必須でないもの | **18 本**（2026-09-11 に実測。**9 本増えた** —— `ci-unity` に足した `Unity Graphics (Linux)` / `Unity DnnEditMode (Linux)` / `Unity DnnStandalone (Linux)` / `UPM tarball install (Linux)` / `Publish the benchmarks` の 5 本と、game-ci が `checkName` から作る `Graphics results` / `DnnEditMode results` / `DnnStandalone results` / `Tarball results` の 4 本。**必須にするかは決めてある —— しない**。理由と昇格の条件は `docs/roadmap.md` の「GPU 経路を CI に載せる」にある）。それ以前の 9 本は次のとおり（PR #74 の 33 checks から必須 24 本を引いて実測）。**`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、`codeql` の集約 check `CodeQL` の 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`ci-unity` の `EditMode results` / `Standalone results` の 2 本（PR では skip される）。** **M6 が足した Web の 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）は 2026-09-10 に必須へ昇格した** —— モバイルの 2 本と同じ手順である（根拠は上の行に 1 度だけ書いてある）。**したがって Web のレーンは、いまは赤ければ merge を止める。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
