...
| `tools/opencv-config.psd1` | OpenCV ビルド構成の唯一の定義元。**`Modules` は現在 `core imgproc imgcodecs objdetect features calib` の 6 つ**（`calib` は 2026-09-02 に足した。構成ハッシュが変わるので、触ると 5 platform 分の再ビルドになる）。`Toolchains`（platform 別 generator/architecture）と `PlatformCMakeArgs`（platform 固有 flag）+ 共通の module allowlist・CMake flags（M3 Task 1 で platform 別化） |
...
| `docs/api-reference.md` | C ABI と C# 公開 API（`CvMat`/`CvOps`/`CvCodecs`/`CvQrCode`/`CvFeatures`/`CvGeometry`/`CvCalibration`/`CvNative`/`TextureConverter`/`NativeArrayExtensions`）のリファレンス（M3 Task 7。M3.5 で `ocvu_imencode` / `ocvu_imdecode` と `CvCodecs` が加わった。M5 で `CvCalibration` が加わった） |
...
- **mobile 実機の挙動。** Android / iOS は CI がビルドし、全部入りにも入るが、
  **実機で動かした実績が 1 度も無い**（M4 の完了条件 4 件が閉じていない。
