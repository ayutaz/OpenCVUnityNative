...
| M7 | Optional profiles と性能。**OpenCV 5 の新しい DNN エンジンはここ** —— 競合が持たない最大の差になりうるが、Unity には推論の代替があるので前倒ししない。**2026-08-30 の上流調査で「5.0 で DNN を作り込むと 5.1 で作り直しになる」根拠が付き、native bridge を module 単位に分ける決定を入れた**（`core`/`imgproc`/`imgcodecs` を安定 ABI として先行、`dnn` は別 target・別 assembly。**CUDA / cuDNN は再配布条件を確かめるまで着手しない**）。詳細と一次情報は roadmap の M7 節 |
