...
| 場所 | 内容 |
| --- | --- |
| `bindings/spec/*.json` | **境界の正本（M5）。** module ごとに 1 ファイル（`infra` / `core` / `imgproc` / `imgcodecs` / `objdetect` / `features` / `geometry` / `calib` / `stereo`）。`schema.json` が形を縛る。**entry 数を数えるのは `docs/api-map.md` の冒頭だけ**である。**ここに 1 エントリ書いて `dev.ps1 generate` を実行するのが、関数を足す唯一の経路である** |
...
| `native/include/opencv_unity_native.h` | 公開 C ABI ヘッダの入口。`OCVU_STATUS_LIST`（status code の唯一の定義元）・handle と struct の型・`OCVU_*` 定数を持ち、下の 9 つを include する。**M5 以降、関数宣言はここに 1 本も無い**（足しても次の `generate` で消える） |
...
| `Packages/com.ayutaz.opencv-unity-native/Runtime/Interop.Dnn/` | **非既定 profile（`dnn`）の宣言先（M7b）。** spec の `profile` が `dnn` の module があれば、生成物 `NativeMethods.<module>.g.cs` はここへ、クラス名は `NativeMethodsDnn`、assembly は `CvUnity.Interop.Dnn` になる（規約は `docs/abi-ownership-and-versioning.md` §4）。asmdef は `defineConstraints: ["OCVU_PROFILE_DNN"]` を持ち、既定ビルドには含まれない。**commit された状態では中身が空である** —— `bindings/spec/dnn.json` がまだ無く、`profile: "dnn"` の module も無い。define を立てて Unity 自身に「実際にコンパイルされ、外すと消えるか」を問う検査は `tests/UnityProject/Assets/Tests/EditMode/ProfileGatingTests.cs` にある。**生成物が実際にコンパイルできることは 2026-09-06 まで誰も確かめていなかった** —— 確かめられていたのは「`defineConstraints` が**手書きの**コードを切る」ことまでで、その穴を通って欠陥が 1 件入っていた（生成物の `[DllImport(LibraryName, ...)]` が `LibraryName` を見られず CS0103。**非既定 profile は別 assembly・別型なので、手書きの `NativeMethods.cs` にある `internal const` が見えない**）。いまは生成器がその 5 行を `#if` 分岐ごと複製し、写し 2 つを `Ocvu.Generator.Tests` の `ProfileTests` が読み比べ、**生成物そのものをコンパイルする検査**が速いレーン（`tools/tests/BindingGenerator.Tests.ps1`）に在る。切り出さなかった理由は `docs/abi-ownership-and-versioning.md` §4 |
...
- `docs/abi-ownership-and-versioning.md` — `Mat` と Unity メモリの所有権契約、`OCVU_ABI_VERSION` の versioning 規約、**リンクしている 8 module の** API allowlist の正本
...
| リンクしている 8 module の API allowlist | M2 の 9 本に、M3.5 / M5 / 2026-09 の API 拡張の追加を足したもの。**関数名をここに並べない** —— 1 本足すたびにこの行だけが古くなる。**内訳は同文書 §3.5〜§3.13、本数は §3 の冒頭が数える** | 同 §3・§3.5〜§3.13 |
