...
## マイルストーン（現在地: **M3.5 は完了**（6 件すべて、2026-08-30）。**v0.2.0 が最新の公開版で、OpenUPM にも載っている。** 次は M4 —— **計画書がまだ無いので `superpowers:writing-plans` で先に書く**。詳細は roadmap）
...
| 必須でないもの | **`ci-unity` の `Plugin <platform>` 4 本と、`ci-native` の `Android arm64-v8a (cross-build)` / `iOS arm64 (cross-build)` の 2 本**（前者は 2026-08-30、後者は M4）。**どちらも必須にしないと決めた。** `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は、この構成の CI がまだ 1 度も緑になっていないので必須にできない** —— 安定して緑になるのを見てから加える、というのが M3 と M3.5 で 2 度穴を開けて学んだ順序である。`build-opencv` / `nightly` / `release` は PR では起動しない（構成変更・定期実行・tag が trigger）ので、そもそも必須にできない | — |
