...
| M6 | Web / Wasm（Unity 同梱 Emscripten と整合）。**着手する前に読むこと** —— roadmap の M6 節に「M4 / M5 の後に着手するとき、何が変わっているか」を足した（platform は 17 箇所・宣言は生成物・依存 allowlist が新しい依存を捕まえる）。****配るのは着手する前ではなく後になった**（2026-09-03 の決定。上記）** |
...
| 必須でないもの | **`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、そして M6 が足した 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）。** **M6 の 3 本を必須にしないのは、この構成の CI がまだ 1 度も緑になっていないからである** —— `CLAUDE.md` の規律は「**安定して緑になったのを見てから必須へ加える**」で、**モバイルの 2 本も M4 の直後は必須外で、2026-08-31 に昇格した。** 同じ手順を踏む（**PR と main で安定して緑になったら昇格する**）。**`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
