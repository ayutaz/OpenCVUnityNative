...
| `core` / `imgproc` / `imgcodecs` / `objdetect` / `features` API の allowlist | Mat の create / release / clone / get_info / copy_from_buffer / copy_to_buffer と cvtColor / resize / GaussianBlur（M2 の 9 本）に、M3.5 の imencode / imdecode、M5 の qr_encode / qr_decode / orb_detect / find_homography を足した 15 本 | 同 §3・§3.6 |
