...
## マイルストーン（現在地: **M3.5 は完了**（6 件すべて、2026-08-30）。**v0.2.0 が最新の公開版で、OpenUPM にも載っている。** **M4（Mobile）は実装が済み PR #41 で CI が緑になっている**（計画は `docs/superpowers/plans/2026-08-30-m4-mobile.md`）。**完了条件 9 件のうち 4 件は閉じていない** —— iOS 実機 / lifecycle / macOS 上の Unity / Windows IL2CPP の結論。詳細は roadmap）
...
| 必須チェック | **19 本。** `ci-native` の 3 job（`Windows x64 (L1 + L3)` / `macOS arm64 (L1 + L3)` / `Linux x64 (L1 + L3)`）、`ci-sanitizers` の 2 job（`Windows x64 AddressSanitizer (L2)` / `Linux x64 ASan+LSan (L2)`）、`ci-lint` の 4 job（`Workflows (actionlint)` / `Shell scripts (shellcheck)` / `PowerShell (PSScriptAnalyzer)` / `Documentation links`）、`codeql` の 2 job（`Analyze c-cpp` / `Analyze csharp`）、`ci-unity` の 2 job（`Unity EditMode (Linux)` / `Unity Standalone (Linux)`）、**`release` の 6 job（`Package windows-x64` / `macos-arm64` / `linux-x64` / `android-arm64` / `ios-arm64` と `Assemble the release assets`）**。2 本 → 5 本（M3 完了時）→ 13 本（2026-08-29、PR #30 が 13 本すべてを緑にしたのを見てから拡張）→ **19 本（2026-08-31。`release.yml` を PR でも走らせるようにし、main と PR の両方で緑になったのを見てから拡張）**。2026-08-31 に API で実測し、この表と一致 | 全部 pass しないと merge できない |
| 必須でないもの | **`ci-unity` の `Plugin <platform>` 4 本と、`ci-native` の `Android arm64-v8a (cross-build)` / `iOS arm64 (cross-build)` の 2 本**（前者は 2026-08-30、後者は M4）。**どちらも必須にしないと決めた。** `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本について、当初の理由（この構成の CI がまだ 1 度も緑になっていない）は 2026-08-31 に失効した** —— M4 以降 PR と main で安定して緑である。**必須にするかは未決**で、決めるまではここに「未決」と書いておく（黙って必須外に置き続けない）。**`release` の `Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
