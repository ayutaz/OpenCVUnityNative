...
| 必須チェック | **24 本。** `ci-native` の 3 job（`Windows x64 (L1 + L3)` / `macOS arm64 (L1 + L3)` / `Linux x64 (L1 + L3)`）、`ci-sanitizers` の 2 job（`Windows x64 AddressSanitizer (L2)` / `Linux x64 ASan+LSan (L2)`）、`ci-lint` の 4 job（`Workflows (actionlint)` / `Shell scripts (shellcheck)` / `PowerShell (PSScriptAnalyzer)` / `Documentation links`）、`codeql` の 2 job（`Analyze c-cpp` / `Analyze csharp`）、`ci-unity` の 2 job（`Unity EditMode (Linux)` / `Unity Standalone (Linux)`）、**`release` の 6 job（`Package windows-x64` / `macos-arm64` / `linux-x64` / `android-arm64` / `ios-arm64` と `Assemble the release assets`）、`ci-native` のモバイル 2 job（`Android arm64-v8a (cross-build)` / `iOS arm64 (cross-build)`）、M6 の Web 3 job（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）**。2 本 → 5 本（M3 完了時）→ 13 本（2026-08-29、PR #30 が 13 本すべてを緑にしたのを見てから拡張）→ **21 本（2026-08-31。`release.yml` を PR でも走らせるようにして 6 本、モバイルのクロスビルド 2 本を足した。どちらも main と PR の両方で安定して緑になったのを見てから拡張した）** → **24 本（2026-09-10。M6 が足した Web の 3 本を昇格した —— #63 以降に merge された 11 本の PR（#63〜#70 と #72〜#74）すべてで 3 本とも success になり、「緑の実績が浅い」という当初の理由が失効したため）**。2026-09-10 に API で実測し、この表と一致 | 全部 pass しないと merge できない |
| 必須でないもの | **18 本**（2026-09-11 に PR #76 で実測 —— **全 check が完了してから** `gh pr checks` の 42 件と保護設定の 24 件を集合演算で突き合わせた）。**9 本増えた** —— 足した job 5 本（`Unity Graphics (Linux)` / `Unity DnnEditMode (Linux)` / `Unity DnnStandalone (Linux)` / `UPM tarball install (Linux)` / `Publish the benchmarks`）と、game-ci が `checkName` から作る check run 4 本（`Graphics results` / `DnnEditMode results` / `DnnStandalone results` / `Tarball results`）である。**必須にするかは決めてある —— しない**（理由と昇格の条件は `docs/roadmap.md` の「GPU 経路を CI に載せる」）。それ以前の 9 本は `Plugin <platform>` 5 本、`Publish the release` 1 本、集約 check `CodeQL` 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`EditMode results` / `Standalone results` 2 本である。**測る時刻に注意する** —— game-ci の `* results` は job の終盤に作られるので、**走っている途中に数えると出てこない**（実際この行は 1 度 12 本と書いて直した。36 件の時点で数えていた）。**`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、`codeql` の集約 check `CodeQL` の 1 本（言語ごとの `Analyze c-cpp` / `Analyze csharp` を個別に必須にしてあるので、集約を足しても止める力は増えない）、`ci-unity` の `EditMode results` / `Standalone results` の 2 本（PR では skip される）。** **M6 が足した Web の 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）は 2026-09-10 に必須へ昇格した** —— モバイルの 2 本と同じ手順である（根拠は上の行に 1 度だけ書いてある）。**したがって Web のレーンは、いまは赤ければ merge を止める。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
