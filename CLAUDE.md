...
| `./tools/dev.ps1 test-unity-editmode` | L4（Unity EditMode、Mono、**47 件**） | 約 27 秒（増分。2026-08-28 実測。**Unity 6000.0.82f1・EditMode 10 件のときの値。** その後 M3.5 で 16 件、M4 で 33 件、M5 で 34 件になったが所要時間は取り直していない） |
| `./tools/dev.ps1 test-unity-player` | L5（Unity IL2CPP Player のビルドと実行、**32 件**） | 約 54 秒（増分・キャッシュ温状態。2026-08-28 実測。cold 実測はまだ無く、roadmap の想定は 5〜20 分。**Unity 6000.0.82f1・10 件での値**） |
