...
| `./tools/dev.ps1 test-unity-web` | **M6。** WebGL の Player を建て、**ブラウザで走らせる**（Playwright + headless Chromium）。先に `assert-emscripten-version.ps1` が **Unity 同梱の Emscripten を対応表と突き合わせる** —— **このレーンに置くのが正しい**（WebGL の Player を建てられる時点で Unity と WebGL 支援は必ず在るので、「道具が無いから飛ばす」経路が構造的に生まれない）。**stripping は有効のまま**にする（無効にすると到達性テストが確かめたいことを確かめられない） | — |
...
| 必須でないもの | **`ci-unity` の `Plugin <platform>` 5 本、`release` の `Publish the release` の 1 本、そして M6 が足した 3 本（`ci-native` の `Web wasm32 (cross-build)` / `ci-unity` の `Web browser E2E` / `release` の `Package web-wasm`）。** **M6 の 3 本を必須にしないのは、緑の実績がまだ浅いからである** —— **2026-09-03 に PR #63 で 3 本とも初めて緑になった**（それまでは 1 度も緑になっていなかった）が、`CLAUDE.md` の規律は「**安定して緑になったのを見てから必須へ加える**」で、**1 回は「安定して」ではない。** **モバイルの 2 本も M4 の直後は必須外で、2026-08-31 に昇格した。** 同じ手順を踏む（**PR と main で安定して緑になったら昇格する**）。**帰結を直視すること: いま Web のレーンは赤くても merge を止めない。****`Plugin` が 4 本から 5 本になった**のは web-wasm が加わったためである。 `Plugin` は落ちると `unity` job が `if: !cancelled()` で走って材料が無いまま赤くなるので、必須チェックである Unity の 2 レーンがそのまま止める（**skip は required check を通す**ので `needs:` だけでは止まらない —— そこは workflow の構造で塞いである）。**モバイルの 2 本は 2026-08-31 に必須へ昇格した** —— 当初の理由（この構成の CI がまだ 1 度も緑になっていない）が失効し、M4 以降 PR と main で安定して緑になったためである。**`Publish the release` は必須にしない** —— PR では常に skip される job で、**GitHub は skip を合格として通す**ので、必須にしても何も止めない。「見ているが止めない」の典型である。`build-opencv` / `nightly` は PR では起動しない（構成変更・定期実行が trigger）ので、そもそも必須にできない | — |
...
- **Web のレーンが赤いこと。** M6 が足した 3 本は**まだ必須チェックではない**
  ので、赤くても merge できる。何が必須で何がそうでないかの記載場所は
  下の表 1 行だけで、正本はさらにその先の GitHub 側の設定である。
  **Web の browser E2E は実物のブラウザで動くが、動かすのは Linux の
  headless Chromium 1 つだけ**である —— 他のブラウザ・実機の挙動は見ていない
