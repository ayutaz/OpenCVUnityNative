...
| `check-unityengine-leak.sh` | PostToolUse (Write/Edit) | **shim がコンパイルする Runtime のフォルダ**への `UnityEngine` 混入を指摘。**一覧を写さず正本から読む** —— 正本は `tests/Managed/CvUnity.Runtime.Shim/CvUnity.Runtime.Shim.csproj` の `<Compile Include>` 行である。**写していた頃は実際に古くなった**: M7c が profile 分離で `Runtime/Interop.Dnn` と `Runtime/Dnn` を足し、shim は 4 フォルダをコンパイルするようになったのに hook は 2 のままで、**後の 2 つは素通りしていた**（2026-09-10 に 4 フォルダへ probe を当てて実測し、正本から読む形に直した）|
