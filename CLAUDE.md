...
| `check-staged-generated-file.sh` | PreToolUse (Bash/PowerShell) | **stage された生成物に検査用の注入が残ったまま commit するのを止める**（2026-09-03 に足した）。`prove-a-check-works` の手順で生成物を壊し、**戻す前にコミットして必須 21 本のうち 3 本を赤くした**のを受けている。**既存の門はどれも止めなかった** —— `block-bulk-git-add.sh` は `-A` と `.` を止めるが **`-u` は素通し**し（コメントは「生成物、一時的なデバッグ変更を巻き込む」と書いているのに実装がずれていた）、`check-generated-file-edit.sh` は **hook の外で書き換えたもの**（`sed` の注入）を見ない。**`-u` だけを塞ぐ手は採らなかった** —— `git commit -a` が同じ形で残り、**片方だけ塞ぐと塞がれていない動詞に移るだけ**である。動詞ではなく結果を見る。**対象を一覧で持たない**（生成物が先頭 5 行で名乗る規約を見る）ので、生成物が増えても変わらない |
