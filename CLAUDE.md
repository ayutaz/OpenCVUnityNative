...
| `tools/run-benchmarks.ps1` | Unity の結果 XML から `OCVU_BENCH:` 行を集めて `artifacts/benchmarks/latest.json` へ書く（M7a）。**時間の速い・遅いは判定しない** —— 時間の閾値は共有ランナーの上でフレークになるので、「測れたか」だけを見る: **XML ごとに**（M7a のレビューで直した。連結後に見ると片方のレーンの benchmark 本体が消えても気づけない）1 本以上拾えたか、そして各値が 0 マイクロ秒でないか（測定が効いていない値を publish しない）。**ログではなく結果 XML の `//output` を読む** —— このリポジトリの確立した型は `TestContext.WriteLine` で、NUnit3 がそれを結果 XML の `<output>` ノードへ入れる |
