...
| `native/include/opencv_unity_native.h` | 公開 C ABI ヘッダの入口。`OCVU_STATUS_LIST`（status code の唯一の定義元）・handle と struct の型・`OCVU_*` 定数を持ち、下の 6 つを include する。**M5 以降、関数宣言はここに 1 本も無い**（足しても次の `generate` で消える） |
