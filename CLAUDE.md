...
| `./tools/dev.ps1 test-unity-graphics` | **M7a。ローカル専用**。`RenderTexture` を使う GPU 依存の経路（`GraphicsTests` / `GraphicsBenchmarkRunner`）を EditMode で検証する。`-nographics` を**付けない**（`-nographics` の下では `graphicsDeviceType` が `Null` になり `RenderTexture.Create()` が true を返しつつ画素が読めなくなるため、実測）。**`ci-unity.yml` からは呼ばれない** —— このレーンが赤くても merge は止まらない。CI から見えないテストの一覧は `tests/UnityProject/Assets/Tests/EditMode/CiVisibilityTests.cs` が名指しで固定している（**一覧は同ファイルの `ExpectedGraphicsOnlyTests` が持つ** —— 本数をここに写すと、1 件足した日にこの行だけが嘘になる） | 未計測 |
| `./tools/dev.ps1 test-unity-tarball` | **UPM tarball の導入検証。** tarball だけを指した使い捨ての Unity プロジェクトを作り、そこで EditMode を走らせる。`-testCategory '!Graphics'` で `test-unity-graphics` 側のテストを除外する（`Test-UnityEditMode` と同じ `$script:UnityGraphicsExclusionCategory` を使う——2026-09-10 まで直書きのコピーで抜けており、GPU に依るテストが「スキップ」ではなく「失敗」していた）。**`-PluginSource` に他 platform の plugin 木（';' 区切り）を渡すと全部入りとして固めてから検証する** —— 渡さなければ従来どおり 1 platform 分で走り、**黙って全部入りのふりはしない** | 約 3 分（2026-08-28 実測。Library/ を持って行かないので Unity が import からやり直す。**Unity 6000.0.82f1・1 platform でのもの**）。**6 platform の全部入りは 2026-09-03 に実測** —— `release.yml` の空撃ちが作った実物の tarball を展開して材料にし、`native plugins present: 6` / `34 passed` / exit 0（**所要時間は取っていない**）。**2026-09-10 に Graphics カテゴリの除外を足した後で実測し直した** —— 除外前は `66 passed=60 failed=6`、除外後は `59 passed` で exit 0（`unity-tarball.xml` の `testcasecount`）|
...
| `tests/UnityProject/Assets/Tests/EditMode/CiVisibilityTests.cs` | **CI で走らないテストの一覧を名指しで固定する**（M7a）。`test-unity-graphics`（GPU に依る `RenderTexture` の経路）は `ci-unity.yml` からは呼ばれないので、`[Category("Graphics")]` を付けたテストは CI から完全に見えなくなる。**列挙そのものが契約である** —— このリポジトリは一般に列挙を嫌うが（`check-generated-file-edit.sh` の設計）、ここは例外で、一覧は「CI が見ていないもの」であり短いほうがよく、伸びたら人が気づくべきものだから列挙で持つ。**`Graphics` と `!Graphics` は補集合なので実行時の和集合は常に完全**であり、穴は「テストを見落とすこと」ではなく「一方のレーンが CI に無いこと」である。**graphics レーンを CI に配線できたら、この一覧は空にでき、そのとき検査ごと消してよい** |
...
- **`RenderTexture` を使う公開 API の GPU 経路（M7a）。**
  `RenderTextureConverter.ToMat` / `RequestMat` が**実際に画素を運ぶ経路**は
  **CI で 1 度も実行されない** —— 検証は `test-unity-graphics` が
  持つが、これはローカル専用で `ci-unity.yml` からは呼ばれず、
  `test-unity-player` / `ci-unity` の 2 レーンは `-nographics` で走るので
  この経路を通れない。**引数の検証だけは CI が通る** ——
  `RenderTextureChecks.ToMatRejectsNull` が `ToMat(null)` を EditMode と
  Standalone の両レーンで呼んでいる。**通っていないのは GPU が要る側である。**
  **前 2 者（tarball / web）は「別の経路で同じものを
  見ている」レーンだが、こちらは配る package に入る公開 API の中身が
  どの経路からも見られていない**という点で 1 段重い。`[Category("Graphics")]` を
  付けたテストの一覧は
  `tests/UnityProject/Assets/Tests/EditMode/CiVisibilityTests.cs` が固定している。
  **`dev.ps1 benchmark` も同じくどの workflow からも走らない**
