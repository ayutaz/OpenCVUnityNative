...
`llvm-strip --strip-unneeded` の効果は、**Android の `.so` が
258,995,040 → 24,385,832 バイト、全部入り tarball が
124,102,343 → 77,528,652 バイト**（strip の有無だけを変えて測った）。

**この版の実物は 77,530,189 バイト（約 74 MB）である。** 直前の
`77,528,652` は strip の効果を測るための再現なので、**配る物そのものの
大きさとは別に書いてある** —— ビルドごとに数十バイト動く。
**v0.3.0 の実物は 69,565,901 バイト（66 MB）**なので、`dnn` をまるごと足しても
増加が約 8 MB で済んでいるのは、この strip が相殺しているからである。
