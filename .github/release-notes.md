<!--
  ...
  **いまの中身は v0.3.0 のもので、2026-09-04 に公開済みである。**
  **次の版を出すときは、ここを書き直すところから始めること** ——
  書き直さずに tag を打つと、**前の版の説明が付いた Release が出る。**

  書き直すときに必ず見る箇所:
    - 「## 前の版（v0.X.Y）から変わったこと」の版番号と中身
    - 「## この版の範囲」の platform 一覧と制限
    - 「## この版で確かめていないこと」
    - 「## 同梱物」の asset 数
    - **絶対 URL の版**（`blob/v0.3.0/...`）—— tag に貼り付くので、
