...
- Desktop、Android、iOS、将来の Web を一貫した UPM パッケージで扱う
...
この比較は「絶対的に競合が存在しない」という市場主張ではない。少なくとも本調査で比較した主要候補には、OpenCV 5、Unity 専用設計、Apache-2.0、UPM、モバイル、将来の Web 対応を一度に満たす選択肢がない、という判断である。
...
もう片方が素通しする）。**全部入りの tarball は 9.6 MB（9,608,334 バイト）**（2026-08-30、CI 実測）なので、
...
（2026-08-30、CI が v0.2.0 の内容で組んだ物の実測。`imgcodecs` を実際に
リンクしたぶん v0.1.1 の 3 platform 合計より大きい）で、どちらも上限まで 50 倍以上ある。**モバイルを足した後の値は
