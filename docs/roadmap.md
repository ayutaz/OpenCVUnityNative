...
### 配布 その 4 — v0.3.0（2026-08-31 の下書き。**2026-09-03 に破棄して打ち直すと決めた。まだ破棄していない**）
...
### 配布 その 5 — v0.3.0（**2026-09-04 に公開した**）
...
>   配信していたのは `0.2.0` だけ）
> - **OpenUPM は tag の移動を追随した。** レビューが「確かめるまで分からない」と
>   した点だが、**打ち直した直後に pipeline のレコードが `cadd52f` → `03a4557` に
>   更新され、probe 回数もリセットされた**（16 → 5）。**配信されるまでには
>   時間がかかる** —— v0.2.0 のときも数時間かかった
>
...
- [ ] 公開後に「最新の公開版」の記述を更新する（**一覧はやること 10 にある** —— ここに写すと 2 つが食い違う）
...
（完了）          （完了）             （完了）              （完了 / v0.1.1）
                        ...
                        （5 件すべて達成。objdetect / features / geometry / calib）
                                            ...
                                            （5 件すべて達成。ブラウザで実測）
                                                              ...
                                                              v
                                                        配布（版は未定）
                     （M4 / M5 / M6 の成果をまとめて届ける。2026-09-03 に
                      「M6 の後にまとめる」と決め、v0.3.0 として着手した）
                                                              |
                                                              v
                                                        M7 profiles
```
