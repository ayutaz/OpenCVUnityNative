...
- [ ] **OpenUPM が `0.4.0` を配信する。** 公開直後は `dist-tags.latest = 0.3.0` の
      まま（`versions = ['0.2.0','0.3.0']`）で、pipeline は公開 74 分前の probe による
      `state=3` / `reason=904` / `commit=6713e51`。**v0.3.0 のときは公開の約 4 時間後に
      切り替わった。** ここで見るのは 2 つ —— レジストリの `dist-tags` と、
      pipeline の `state` が 2 になること。**6〜12 時間動かなければ OpenUPM 側に
      再ビルドを頼む。**

      **v0.3.0 と違う点が 1 つある**: あちらは**打ち直して commit が変わった**ことで
      拾い直された。今回は本文だけ差し替えたので **`commit` は `6713e51` のままである。**
      **commit が変わらないまま拾い直すかは、確かめるまで分からない。**
