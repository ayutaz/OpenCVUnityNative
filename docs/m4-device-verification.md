...
**~~`v0.3.0` の下書きを公開してはならない。~~ この警告は 2026-09-04 に失効した。**
指していた下書きは M5 が main に入る前に作ったもので生成物が 1 つも入っておらず、
**下書きと tag は破棄して `03a4557` で打ち直し、v0.3.0 として公開済みである**
（roadmap の「配布 その 5」）。**その v0.3.0 も、もう最新ではない** ——
**2026-09-12 に v0.4.0 を公開した**（roadmap の「配布 その 6」）。
**この手順書が対象にすべきは、常に最新の公開版である。**

**この手順書は `dnn`（M7c）を扱っていない。** M7 で `dnn` が opt-in profile として
入ったが、実機での確認手順はここに無い —— **`dnn` も実機で 1 度も動かしていない。**
**v0.4.0 でその `dnn` が利用者に届いた**ので、**手順が無いまま配った状態**である。
