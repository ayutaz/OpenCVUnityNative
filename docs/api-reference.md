...
mobile と Web の現況（何がビルドされ、何が実機で動いていないか）は
あちらに 1 箇所だけ書いてあるので、**ここには写さない。**
...
****このレーンが merge を止めるかどうかは `CLAUDE.md` の「機構として強制されて
いること」が持つ**（正本はさらにその先の GitHub 側 branch protection の設定で
ある）—— ここに写すと、昇格した日にこちらだけが静かに嘘になる。**
