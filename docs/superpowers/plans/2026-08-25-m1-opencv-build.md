...
> **状態: 実施済みの計画である。** 2026-08-26 に PR #4（`9e4b5b5`）として main へ入った。
> **完了判定は [docs/roadmap.md](../../roadmap.md) の M1 節にある** —— 完了条件も、
> 実測との照合もそこが正本である。実施当時の作業メモは手元の `.superpowers/` 配下に
> あるが、**これは `.gitignore` で追跡外なのでクローンには入らない。** 別のマシンで
> この計画書を読む者には検証できないため、参照先として当てにしないこと。
> roadmap の M1 節には**意図的に見送った検証**（送った CMake flag を成果物が守ったかを
> 見ない）が「既知の欠陥」として残っており、その担当は M3 へ移した。
>
> **本文のチェックボックス 43 個は 1 つも更新していない。** 実施当時に印を付けなかった
