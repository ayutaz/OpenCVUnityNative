...
> **3 計画とも実行済みである（(a) / (b) は `be5615f`、(c) は `e1b0930`）。
> 以下は着手前の記録として読むこと。** M7 の完了条件 5 件はすべて満たされ、
> 判定は `docs/roadmap.md` の `M7a の判定` / `M7b の判定` /
> `CUDA / cuDNN 同梱の判定` / `M7c の判定` の 4 節に分かれている（索引は
> 同文書の `M7 の判定`）。**M7 はこのリポジトリで最後のマイルストーンで、
> 後続は無い** —— この文書が「後の計画へ送る」と書いた項目には、送り先が存在しない。
...
**~~(c) は (b) が済むまで着手してはならない~~** —— roadmap の決定 4 が明文でそう定めていた。
**この gate は満たされた**（(b) は 2026-09-06、所有者による解除は 2026-09-08。roadmap の
「この決定は解除された」）。
...
| 5 | CUDA / cuDNN を同梱するなら、再配布条件の確認が済んでいること。確認できないなら**同梱しないと決めて記録する** | ~~**(c)**~~ → **計画を経ない文書上の決定**（`docs/roadmap.md` の `### CUDA / cuDNN 同梱の判定`。**この行が (c) を担当と書いていたのは誤りで、(c) の Task 1〜6 に条件 5 の作業は無い。正本は roadmap である**） |
...
- **main で直接作業しない。** 当時のブランチは `feat/m7-profiles-and-performance`（**もう存在しない**）
...
| **`gapi` / `videoio` / 動画 codec** | M7 のゴールには挙がっているが、`dnn` より優先度が低く、FFmpeg / GStreamer の再配布条件を別途確認する必要がある。~~**(c) の後に別計画で扱う**~~ → **担当が無い。M7 の後にマイルストーンは無く、この項目を引き取る計画は存在しない**（やるなら、そのときに新しく計画を立てることになる） |
