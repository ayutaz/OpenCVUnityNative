...
- [API リファレンス](./api-reference.md)
  - C ABI 18 関数（M2 の 9 本 = `Mat` のライフサイクルと buffer 転送の 6 本 + `cvtColor` / `resize` / `GaussianBlur` の 3 本、**M3.5 で足した `imencode` / `imdecode` の 2 本**、**M5 の module 拡張で足した `qr_encode` / `qr_decode` / `orb_detect` / `find_homography` / `undistort` / `find_chessboard_corners` / `calibrate_camera` の 7 本**）と、その上に立つ C# の公開 API
  - **この 18 本は公開 ABI の総数ではありません。** 残りは M0 / M1 由来の 8 本（ABI version の取得、last-error の取得、status 表の照会、OpenCV version / build information）と `ocvu_debug_crash` 1 本で、合わせて 27 本です（**本数を数える正本は [API 対応表](./api-map.md) の冒頭**）。
...
- [C ABI の所有権と versioning](./abi-ownership-and-versioning.md)
  ...
  - API allowlist（M2 の 9 本 + M3.5 の `imencode` / `imdecode` + M5 の module 拡張 7 本で 18 本）と、まだ作らないもの
