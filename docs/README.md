...
M3.5（配布の形と、実用に必要な最小の穴）は、roadmap の完了条件 6 件のうち **4 件を満たし、1 件が「満たすが未実証」、1 件が部分達成**です。**CI は 14 本すべて green で main に入りました**（PR #34、`41cda19`、2026-08-29。Unity のレーンは CI 上でも `==> [EditMode] 16 passed` / `==> [Standalone] 10 passed`）。
