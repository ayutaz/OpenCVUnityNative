...
M3.5（配布の形と、実用に必要な最小の穴）は、roadmap の完了条件 6 件のうち **4 件を満たし、1 件が「満たすが未実証」、1 件が部分達成**です。**CI は green で main に入りました**（必須 13 本 + PR にだけ出る CodeQL の集約で 14 本）（PR #34、`41cda19`、2026-08-29。Unity のレーンは CI 上でも `==> [EditMode] 16 passed` / `==> [Standalone] 10 passed`）。

**「満たすが未実証」は条件 3 です。** 全部入りの中で Unity が自分の platform の binary だけを読むことを見る検査は書きましたが、**自動で走る唯一の場所（`ci-unity.yml`）には Linux の plugin 1 つしか無く、そこでは 6 件が要素 1 個の集合を検査して緑になります**。取り違えが起こりうる状況が CI では成立しないので、「満たす」に数えていません（M2 の条件 7 に当てたのと同じ基準です）。**部分達成は条件 4** で、OpenUPM の登録申請だけが残っています。
