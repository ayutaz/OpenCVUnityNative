...
M3（Desktop 3 platform と配布の再現性）は着手済みで、Task 1〜7（構成の platform 別化、CMakePresets の 3 platform 拡張、Windows/macOS/Linux の linkage 検証、CI の 3 platform 拡張、配布物一式、Unity sample と API reference）はすべて実装・コミット済みですが、roadmap の完了条件 6 件のうち実測で満たしているのは 1 件（Unity sample と API reference）だけです。理由はコードの欠落ではなく、実装した 7 commit がまだ CI に一度も通っていないことです——このリポジトリの `ci-native.yml` / `ci-sanitizers.yml` は pull request かメイン branch への push でしか起動せず、この作業ブランチはまだ PR を出していません。macOS/Linux 版の native plugin バイナリと Plugin Import Settings はどこにも存在せず、Linux の LeakSanitizer レーンと macOS/Linux の linkage 検証は一度も実行されておらず、UPM の Git URL/tarball 導入も未検証です。したがって M3 も「完了」とは称していません。詳細は roadmap の M3 節にあります。
