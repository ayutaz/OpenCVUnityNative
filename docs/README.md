...
- [ロードマップ](./roadmap.md)
  ...
  - **差別化の穴**（2026-08-29 の再調査。競合が全員 OpenCV 4 系のままであること、差別化の穴 10 件（掲げている差別化のうち達成していないものと、競合が持っていて本案が持たないものの合算）と、その担当。**解消・部分達成の別は roadmap の表が持つ** —— 判定は今後も動くので、件数をここに写さない）
  - **M7 の上流調査**（2026-08-30。OpenCV 5.1 に向けて `dnn` がどう動いているかを一次情報で確かめ、`dnn` を足す前に native bridge を module 単位に分ける決定を入れた）
  - ローカルループと CI の役割分担、GitHub Actions のワークフロー構成（M3 の後に `ci-lint` / `codeql` / `nightly` と Dependabot を追加。**`nightly` は 2026-08-29 から schedule で毎日走っている** —— **「まだ 1 度も走っていない」と書いてあったのは逆向きの陳腐化で、動いている検査を存在しないことにしていた**。実績は roadmap のワークフロー表）
  - 実装計画: [M0 自動 TDD ハーネス](./superpowers/plans/2026-08-25-m0-tdd-harness.md)（完了）、[M1 OpenCV ビルド](./superpowers/plans/2026-08-25-m1-opencv-build.md)（完了）、[M2 Windows vertical slice](./superpowers/plans/2026-08-26-m2-windows-vertical-slice.md)（完了。8 件中 8 件。条件 7 は game-ci + Linux で満たした — 詳細は roadmap の M2 節）、[M3 Desktop 3 platform と配布の再現性](./superpowers/plans/2026-08-28-m3-desktop-three-platforms.md)（6 件すべて達成。v0.1.0 と v0.1.1 を公開済み——詳細は roadmap の M3 節）、[M3.5 配布の形と最小の穴](./superpowers/plans/2026-08-30-m3.5-distribution-shape.md)（全部入り tarball / `imgcodecs` / Unity 6.3。**6 件すべて達成。v0.2.0 を公開し OpenUPM にも登録済み**——詳細は roadmap の M3.5 節）、[M4 Mobile](./superpowers/plans/2026-08-30-m4-mobile.md)（Android / iOS のクロスビルドと `WebCamTexture` 対応。**9 件中 6 件を満たし、3 件は閉じていない** —— iOS 実機 / lifecycle / macOS 上の Unity（**Windows IL2CPP は「結論を出す」ことが条件だったので満たしている**）。実機が要る 2 件は [実機検証の手順](./m4-device-verification.md) に落としてある）、[M5 binding specification と generator](./superpowers/plans/2026-08-31-m5-binding-generator.md)（spec を正本にして C ヘッダ・C# の P/Invoke・到達性テスト・[API 対応表](./api-map.md) を生成する。**5 件中 4 件を満たし、条件 2 は計画の側で意図的に外してある** —— 詳細は roadmap の M5 節）、[M5 条件 2 の前半 objdetect / features](./superpowers/plans/2026-09-01-m5-modules-objdetect-features.md)（QR コードの符号化・復号と ORB の特徴点検出を出し、条件 2 を「部分的に満たした」にした。続けて `geometry`（射影変換の推定）も出した）、[M5 条件 2 の最後 カメラの歪み補正](./superpowers/plans/2026-09-01-m5-calib-undistort.md)（`ocvu_undistort` / `ocvu_find_chessboard_corners` を `calib` module を足さずに出した。**この時点では係数を求める `cv::calibrateCamera` が無く、条件 2 は「部分的に満たした」のままだった**）、[M5 条件 2 の完了 カメラ校正](./superpowers/plans/2026-09-02-m5-calib-camera.md)（**`calib` module を足し、`cv::calibrateCamera` を出して校正の輪を閉じた。構成ハッシュが変わり 5 platform 分の OpenCV を作り直した**（`4785d98e9aad` → `09fcbe260d87`）—— 詳細は roadmap の M5 節）、[M6 Web / Wasm](./superpowers/plans/2026-09-03-m6-web-wasm.md)（**6 つ目の platform。クロスビルドかつ静的ライブラリ**で、**ブラウザで実際に走らせて初めて出る欠陥を 7 件捕まえた** —— そのうち「例外バリアが wasm で成立していなかった」が最も重い。**完了条件 5 件すべてを満たした** —— 詳細は roadmap の M6 節）、**[API 拡張（A〜F）—— 実測で覆った前提と、その決定](./superpowers/plans/2026-09-05-api-expansion-corrections.md)（下の 5 本すべてに優先する。実装前に 12 観点 x 2 段で計画の前提を実測し、ブロッカー 11 件・要修正 22 件・見落とし 53 件を潰した記録。**反証段が調査段の修正案そのものを 2 件否定している**）**、[API 拡張（A〜F）の全体設計](./superpowers/plans/2026-09-05-api-surface-expansion.md)（**マイルストーンではない。M6 と M7 の間に挟む API 拡張**で、既にリンク済みの module から 26 本を出す。**OpenCV の再ビルドは 1 度も起こさない**。下の 4 つの計画がこれを分担する）、[Phase 1 姿勢と ArUco](./superpowers/plans/2026-09-05-api-pose-and-aruco.md)、[Phase 2 imgproc の実用関数](./superpowers/plans/2026-09-05-api-imgproc-ops.md)、[Phase 3 core の基本演算](./superpowers/plans/2026-09-05-api-core-ops.md)、[Phase 4 マッチングとステレオ](./superpowers/plans/2026-09-05-api-matching-and-stereo.md)（**`stereo` を 8 つ目のリンク済み module にする唯一の計画**）、**[M7 Optional profiles と性能の設計](./superpowers/plans/2026-09-05-m7-profiles-and-performance.md)（**仕様であって計画ではない。**下の 3 本が共有する正本で、完了条件の担当・Global Constraints・7 つの決定を持つ。**3 本とも実行済みなので、着手前の記録として読むこと**）**、[M7 (a) 低コピー経路と benchmark](./superpowers/plans/2026-09-05-m7a-low-copy-and-benchmarks.md)（**実行済み（`be5615f`）。完了条件 2・3 を満たした。決定的な量は assert し、時間は公開するだけにする。**RenderTexture / AsyncGPUReadback を出し、native texture pointer は「やらないと決めた」として評価だけを記録する。**公開 ABI を 1 本も増やさない**）、[M7 (b) module 分離](./superpowers/plans/2026-09-05-m7b-module-separation.md)（**実行済み（`be5615f`）。完了条件 4 を満たした。**native の CMake と C# の assembly を module 単位に割る。**先に安全網を作る** —— 配布 binary のエクスポートが spec と完全一致することを要求する検査を足してから作り替える。**公開 ABI を 1 本も増やさない**）、[M7 (c) dnn profile](./superpowers/plans/2026-09-05-m7c-dnn-profile.md)（**実行済み（2026-09-08〜10、`e1b0930`）。完了条件 1 を満たした。** `dnn` の C ABI 4 本と `CvUnity.Dnn` を opt-in profile として出した。**「着手してよいという合図ではない」という但し書きは 2026-09-08 の gate lift で失効した** —— 経緯は roadmap の「この決定は解除された」）
- [API リファレンス](./api-reference.md)
  - allowlist に載っている C ABI 関数（M2 の `Mat` のライフサイクルと buffer 転送 + `cvtColor` / `resize` / `GaussianBlur`、**M3.5 の `imencode` / `imdecode`**、**2026-09 の API 拡張で姿勢 / ArUco / imgproc の実用関数 / core の基本演算 / 記述子マッチング / ステレオ視差の 26 本**、**M5 の `qr_encode` / `qr_decode` / `orb_detect` / `find_homography` / `undistort` / `find_chessboard_corners` / `calibrate_camera`**）と、その上に立つ C# の公開 API
  - **allowlist の本数は公開 ABI の総数ではありません。** 残りは M0 / M1 由来の診断用 API（ABI version の取得、last-error の取得、status 表の照会、OpenCV version / build information）と `ocvu_debug_throw` / `ocvu_debug_crash` です（**一覧の正本は [API リファレンス](./api-reference.md) の「この allowlist に含まれないもの」です** —— ここに写すと 1 本増えた日にこの行だけが嘘になります。**実際 `ocvu_debug_throw` が抜けていました**）。**どちらの数も [API 対応表](./api-map.md) の冒頭と [所有権と versioning](./abi-ownership-and-versioning.md) §3 が持ちます** —— ここには写しません。
  - **まだ無い機能は書きません。** 範囲は M2 で確定し、M3.5 の `imgcodecs`、M5 の module 拡張、2026-09 の API 拡張（姿勢 / ArUco / imgproc / core / マッチング / ステレオ）、M7a の `RenderTexture`、**M7c の `dnn`（opt-in profile）**が加わった allowlist に一致します
- [API 対応表](./api-map.md)
...
M0 から M7 までです。**判定が 1 節にまとまっていないのは M7 だけ**で、5 件が
`M7a の判定` / `M7b の判定` / `CUDA / cuDNN 同梱の判定` / `M7c の判定` の 4 節に
分かれています（索引は roadmap の `M7 の判定`）。出したのは、低コピー経路の実測と
benchmark の公開（`RenderTexture` / `AsyncGPUReadback`）、C ABI と C# の module 分離、
CUDA / cuDNN を同梱しないという決定、そして **`dnn` を opt-in profile として出すこと**
（`OCVU_PROFILE_DNN` を立てたときだけコンパイルされる C# の `CvUnity.Dnn`。
native binary には既定で入っています）です。

**満たしたことと、実証したことは同じではありません。** M7 が残した穴を、
利用者に関わるものだけ挙げます —— **`dnn` は実機で 1 度も動かしておらず、推論の
速さも測っていません**。`ocvu_dnn_net_forward` の `.clone()` が要ることを示す
再現テストがなく、**確定させる経路がこのリポジトリの構成上ありません**。
`dnn` が引き込む MLAS / ONNX Runtime は `libopencv_dnn.a` に静的に取り込まれるため
