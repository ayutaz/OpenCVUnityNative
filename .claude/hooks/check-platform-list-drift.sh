...
# （2026-09-10 に測り直し: 6 ファイルが該当し、6 つとも 6/6。
# **実測日を書くのは、この値が platform が増えるたびに古くなるからである**
# ——「6 つとも 5/5」と書いてあった頃は M6 で platform が 6 になった時点で
# 古く、実装は正しく動いていたのに記載だけがずれていた）。3 件未満の
# ファイルは、意図して一部だけを扱っている（例: Linux の移植性検査）ので
# 対象外。
#
