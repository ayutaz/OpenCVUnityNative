...
# 一括 add はそれ以外の意図しないファイル（別作業の途中経過など）まで
# 巻き込む。M0 では全 8 タスクのディスパッチで
# 「変更したパスだけを個別に stage せよ」と手で書き続けた。それを機械化する。
#
# **この hook が止めるのは `-A` / `--all` / `--no-ignore-removal` / `.` /
# `:/` / `:` の完全一致だけである。`git add -u` と `git commit -a` は通す。**
# 理由と実装をずらさないために明記しておく —— 以前この冒頭は「生成物、
# 一時的なデバッグ変更まで巻き込む」と書いていたが、**その 2 つを正確に
# stage する動詞こそ `-u` であり、実装はそれを 1 度も見ていなかった。**
# 分担は意図的である: 生成物が履歴に入るのを止めるのは
# check-staged-generated-file.sh で、そちらは commit の時点で index の
# 結果を見る（動詞を 1 つずつ塞ぐと、塞がれていない動詞に移るだけである
# ——その判断は同ファイルの冒頭が書いている）。
#
