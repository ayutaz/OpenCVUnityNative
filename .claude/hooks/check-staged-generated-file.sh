...
#
# **この規則が覆わない形が 1 つある**（レビューが指摘した限界）:
# **spec と、手で書き換えた生成物を、同じ commit で stage した場合は通る。**
# 「spec を直して generate を走らせ、そのついでに生成物も手で触った」形である。
# **今回踏んだ事故とは別物で**（あれは `bindings/` を一切触っていなかった）、
# **CI の `verify-generated` が確実に捕まえる。**
# **この hook は速いローカルの門であって、門番ではない。**
#
