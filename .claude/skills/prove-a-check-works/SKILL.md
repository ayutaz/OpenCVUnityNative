...
M3.5 の計画書は「macOS の `.meta` に `Editor: OS: AnyOS` を書く」と記録し、
その結果として複数の文書が「3 件落ちる」と書いた。**実際には 2 件しか落ちない。**
検査は `GetEditorData("OS")` を `"Windows"` と比較するので、`AnyOS` では一致せず、
1 件は通ってしまう。`OS: Windows` にすると 3 件落ちる。
