...
**pull request で走るレーンのほとんどが merge を止めます。** 必須チェックは 24 本です: desktop 3 platform の契約・P/Invoke・sanitizer、Android・iOS・Web / Wasm のクロスビルド、lint の 4 job、CodeQL の 2 つ、Unity の 2 レーン、ブラウザでの E2E、そして 6 platform 分の配布物をビルド・組み立てる release の 7 job。12 本は意図的に必須にしていません —— **うち 5 本は 2026-09-11 に増えました**。`ci-unity` に `Graphics` レーン・`dnn` profile の 2 レーン・tarball 導入の job・benchmark を publish する job を足したときのもので、**必須にしないのは意図的です** —— このプロジェクトはレーンが緑であり続けるのを見てから昇格させる方針で、これらの実績は 1 ブランチ上で**多くても 3 run**です（`Graphics` と dnn の 2 レーンが 3 run、tarball が 2 run、benchmark は 1 run）。（**数え方に注意が要ります**: GameCI は `EditMode results` のような check run も commit 上に作りますが、`gh pr checks` はそれを列挙しません。ここの数は `gh pr checks` の結果を、保護設定と名前で突き合わせたものです。）

残る 7 本のうち 5 本は Unity のレーンが消費する platform ごとのプラグインをビルドするもので、**どれかが失敗すると Unity のレーンは走ったうえで材料が無いことで赤くなり**、それが merge を止めます。残る 4 本は、言語ごとの解析を個別に必須にしてある `CodeQL` の集約 check、pull request では skip される Unity の結果公開 job 2 本、そしてこの節の末尾で述べる `Publish the release` です。（Web / Wasm の 3 本（クロスビルド・ブラウザでの E2E・リリースの梱包）は **2026-09-10 に必須へ昇格しました** —— #63 以降に merge された 11 本の pull request すべてで 3 本とも緑になり、そのうち main でも走る 2 本は main の直近 6 run でも緑だったのを見てからです。**したがって Web のレーンも、赤ければ merge を止めます。** 上の 24 本に数えてあります。）**昇格は証拠に従いますが、その証拠は誰かが見に行かないと出てきません。****skip された必須チェックは合格として通る**ので、その守りなしにそれらへ依存すると、壊れたビルドが通ってしまいます。**レーンは安定して緑になってから必須にします** —— ただし過去 2 回踏んだのは、その逆の形でした: **レーンを足したのに必須にしないまま置いたこと**が、「赤いのに merge できる」隙間そのものです。2026-08-29 までは Unity・lint・CodeQL の workflow がすべての pull request で走りながら必須ではなく、**赤いまま merge できました。CI が見ていることと CI が止めることは別で、ゲートなのは後者だけです。** 残る 3 つの workflow（`build-opencv`、`nightly`、`unity-probe`）は pull request では起動しないので、そもそも必須にできません。`release` は 2026-08-31 までその一覧にありました —— いまは pull request でも走ります。tag でしか走らなかった間に配布の経路に欠陥が 3 件たまり、**うち 1 件は「tag を打つとリリースが 1 件も作られない」というものでした。** その `Publish the release` job だけは必須にしません —— pull request では設計上 skip され、**skip は合格として通る**ので、必須にしても何も止まらないからです。
