...
# **2026-09-11 から CI にも同等のものが在ります。ただしどちらの check も
# 必須ではありません**（下の「CI が見ているもの」）。
...
**この表に無いレーンが 4 つあります。CI がその `dev.ps1` のコマンドを直接呼ばないからです。** **4 つとも、いまは CI に同等のものが在ります** —— それが変わったのは 2026-09-11 で、それ以前は 2 つに**相当するものが一切ありませんでした**。結び付きが最も強いのは `test-unity-tarball` で、CI は `./tools/dev.ps1 test-unity-tarball -PrepareOnly` を呼んで使い捨てプロジェクトを作り、起動だけを GameCI に渡します —— **CI が導入する tarball と、あなたが手元で導入する tarball は、同じコードが作ります。** 最も緩いのは `test-unity-web` で、CI の `Web browser E2E` job は Player を自分で建て、`dev.ps1` を通さずに同じ 2 つのスクリプトを直接呼びます。`test-unity-graphics` と `benchmark` はその中間で、CI の `Graphics` レーンと `benchmarks` job が同じものを動かし、同じものを集めます。
...
**pull request で走るレーンのほとんどが merge を止めます。** 必須チェックは 24 本です: desktop 3 platform の契約・P/Invoke・sanitizer、Android・iOS・Web / Wasm のクロスビルド、lint の 4 job、CodeQL の 2 つ、Unity の 2 レーン、ブラウザでの E2E、そして 6 platform 分の配布物をビルド・組み立てる release の 7 job。18 本は意図的に必須にしていません —— **うち 9 本は 2026-09-11 に増えました**。`ci-unity` に `Graphics` レーン・`dnn` profile の 2 レーン・tarball 導入の job・benchmark を publish する job を足したときのもの（job が 5 本と、Unity の 4 つが作る GameCI の結果 check 4 本）で、**必須にしないのは意図的です** —— このプロジェクトはレーンが緑であり続けるのを見てから昇格させる方針で、これらの実績は 1 ブランチ上で**多くても 3 run**です（`Graphics` と dnn の 2 レーンが 3 run、tarball が 2 run、benchmark は 1 run）。（**数えるのは run が終わってからにしてください**: GameCI は結果 check を各 job の終盤に作るので、途中で数えると少なく出ます。ここの数は完了した PR での `gh pr checks` を、保護設定と名前で突き合わせたものです。）

残る 9 本のうち 5 本は Unity のレーンが消費する platform ごとのプラグインをビルドするもので、**どれかが失敗すると Unity のレーンは走ったうえで材料が無いことで赤くなり**、それが merge を止めます。残る 4 本は、言語ごとの解析を個別に必須にしてある `CodeQL` の集約 check、pull request では skip される Unity の結果公開 job 2 本、この節の末尾で述べる `Publish the release`、そしてこの変更より前から在る Unity の結果 check 2 本です。（Web / Wasm の 3 本（クロスビルド・ブラウザでの E2E・リリースの梱包）は **2026-09-10 に必須へ昇格しました** —— #63 以降に merge された 11 本の pull request すべてで 3 本とも緑になり、そのうち main でも走る 2 本は main の直近 6 run でも緑だったのを見てからです。**したがって Web のレーンも、赤ければ merge を止めます。** 上の 24 本に数えてあります。）**昇格は証拠に従いますが、その証拠は誰かが見に行かないと出てきません。****skip された必須チェックは合格として通る**ので、その守りなしにそれらへ依存すると、壊れたビルドが通ってしまいます。**レーンは安定して緑になってから必須にします** —— ただし過去 2 回踏んだのは、その逆の形でした: **レーンを足したのに必須にしないまま置いたこと**が、「赤いのに merge できる」隙間そのものです。2026-08-29 までは Unity・lint・CodeQL の workflow がすべての pull request で走りながら必須ではなく、**赤いまま merge できました。CI が見ていることと CI が止めることは別で、ゲートなのは後者だけです。** 残る 3 つの workflow（`build-opencv`、`nightly`、`unity-probe`）は pull request では起動しないので、そもそも必須にできません。`release` は 2026-08-31 までその一覧にありました —— いまは pull request でも走ります。tag でしか走らなかった間に配布の経路に欠陥が 3 件たまり、**うち 1 件は「tag を打つとリリースが 1 件も作られない」というものでした。** その `Publish the release` job だけは必須にしません —— pull request では設計上 skip され、**skip は合格として通る**ので、必須にしても何も止まらないからです。
