...
**pull request で走るレーンのほとんどが merge を止めます。** 必須チェックは 29 本です: desktop 3 platform の契約・P/Invoke・sanitizer、Android・iOS・Web / Wasm のクロスビルド、lint の 4 job、CodeQL の 2 つ、Unity の 2 レーン、ブラウザでの E2E、6 platform 分の配布物をビルド・組み立てる release の 7 job、そして **2026-09-11 に足した `ci-unity` の 5 job**（`Graphics` レーン・`dnn` profile の 2 レーン・tarball 導入の job・benchmark を publish する job）。13 本は意図的に必須にしていません —— うち 6 本は GameCI の結果 check（`EditMode results` とその 5 つの兄弟）で、**pull request では `neutral` になり、GitHub はそれを合格として通す**ので、必須にしても止める力は増えません。（**数えるのは run が終わってからにしてください**: GameCI は結果 check を各 job の終盤に作るので、途中で数えると少なく出ます。ここの数は完了した PR での `gh pr checks` を、保護設定と名前で突き合わせたものです。）
