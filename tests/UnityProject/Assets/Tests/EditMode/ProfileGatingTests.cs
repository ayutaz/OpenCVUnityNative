...
public class ProfileGatingTests
{
    ...
    /// **綴りを見る。**
    ///
    /// **この検査の役目は 2026-09-11 に変わった。** それまで上の一群は
    /// 「define が立っていない状態で assembly が現れない」だけを見ており、
    /// **`defineConstraints` の値を打ち間違えてもそれは全部真だった**
    /// —— 存在しない define は決して立たないので、その assembly は
    /// どんな define を立てても永久に現れない。「切ってある」と
    /// 「壊れている」の区別が付かなかった。
    ///
    /// いまは `AssertTheAssemblyMatchesTheDefine` が両方向を見るので、
    /// **`DnnEditMode` レーン（define を立てる側）では綴り違いを
    /// `Assert.Contains` が直接捕まえる。** それでもこの検査を残すのは、
    /// **既定のレーンだけを走らせたときに何も捕まらない**からである ——
    /// `DnnEditMode` は必須チェックではないので、綴りが壊れたまま
    /// merge できる経路が残る。ここはどのレーンでも同じことを見る。
    ///
